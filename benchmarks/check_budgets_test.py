...
class BudgetContractTests(unittest.TestCase):
    def run_checker(self, results):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env = os.environ.copy()
            env["FFX_BENCH_RESULTS_GLOB"] = str(result_dir / "*.json")
    ...
    def test_linux_keeps_two_millisecond_raw_budget(self) -> None:
        self.assertEqual(
            check_budgets.command_budget("Linux", "ffx sessions --json"),
            0.002,
        )

    def test_darwin_has_no_local_product_budget(self) -> None:
        self.assertIsNone(
            check_budgets.command_budget("Darwin", "ffx sessions --json"),
        )
