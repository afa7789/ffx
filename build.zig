...
pub fn build(b: *std.Build) void {
    ...
    // Keep the upstream `fx` artifact available to shared CI and integration
    // tooling while `ffx` remains the product binary and public name.
    const upstream_compat_install = b.addInstallArtifact(exe, .{
        .dest_sub_path = "fx",
    });
    b.getInstallStep().dependOn(&upstream_compat_install.step);
    ...
    const run_step = b.step("run", "Run ffx");
    ...
    // Keep upstream's helper tests working while the product binary is named ffx.
    run_exe_tests.setEnvironmentVariable(
        "FX_TEST_PRODUCT_EXE",
        b.getInstallPath(.bin, "ffx"),
    );
