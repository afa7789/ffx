...
This directory owns the non-publishing Stage 1 build for a smaller macOS arm64 `ffx` candidate. It preserves Zig ReleaseSafe semantics and the complete product feature set, then uses native LLVM profiles to keep measured hot code speed-oriented and compile profile-proven cold functions for size.
...
python3 -m scripts.pgso build \
  ...
  --output-dir /tmp/ffx-pgso-build

python3 -m scripts.pgso train \
  --llvm-bin "$(brew --prefix llvm@21)/bin" \
  --output-dir /tmp/ffx-pgso-train

python3 -m scripts.pgso all \
  --llvm-bin "$(brew --prefix llvm@21)/bin" \
  --output-dir /tmp/ffx-pgso-candidate \
...
Corpus scenarios inherit only a small operating-system environment allowlist. Credentials, live-test flags, tracing settings, and repository dotenv files are excluded unless a value is explicitly declared in the versioned corpus. The runner temporarily installs the assigned artifact at `zig-out/bin/ffx` for E2E compatibility, then restores the prior file (or prior absence) after success, failure, timeout, or cancellation.
...
aggregate is packaged as `ffx-macos-aarch64.tar.gz`.
