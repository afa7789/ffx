...
def _installed_training_binary(
    ...
    canonical = corpus.repo_root / "zig-out" / "bin" / "ffx"
...
def _scenario_environment(runtime_home: pathlib.Path) -> dict[str, str]:
    ...
    environment["FFX_E2E_DISABLE_DOTENV"] = "1"
    return environment
...
def run_behavior_corpus(
    ...
    *,
    ...
    def prepare(scenario: Scenario, environment: dict[str, str]) -> object:
        ...
        environment["FFX_TRACE_LOG"] = str(trace_path)
        return None
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Inspect the ffx PGSO corpus")
    parser.add_argument(
        "--manifest",
        required=True,
        type=pathlib.Path,
