...
pub fn logOptions() session_log.Options {
    return logOptionsFromEnv(
        io_mod.getenv("FFX_E2E_SESSION_BOUNDARY"),
        io_mod.getenv("FFX_E2E_SESSION_BOUNDARY_READY"),
    );
}
...
fn pauseAtRequestedBoundary(
    ...
    const requested = io_mod.getenv("FFX_E2E_SESSION_BOUNDARY") orelse return;
    if (!std.mem.eql(u8, requested, @tagName(point))) return;

    if (io_mod.getenv("FFX_E2E_SESSION_BOUNDARY_READY")) |path| {
