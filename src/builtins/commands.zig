...
pub const top_level_specs = [_]TopLevelSpec{
    ...
    .{
        ...
        .options = &.{
            ...
            .{ .flag = "--full-access", .description = "Disable ffx permission checks" },
    ...
    .{
        ...
        .summary = "Sign in with Codex or Grok",
    },
    .{
        .kind = .logout,
        .token = "logout",
    ...
    .{
        ...
        .details = &.{
            ...
            "  ffx mcp add --transport http NAME URL",
            "  ffx " ++ command_specs.mcp_auth_usage,
            "  ffx mcp list [--connect]",
            "  ffx mcp logout NAME",
            "  ffx mcp path",
            "  ffx mcp remove NAME",
    ...
    .{
        ...
        .summary = "Show local ffx token usage and spend",
        .options = &.{
            .{ .flag = "--period <24h|7d|30d>", .description = "Select a rolling window (default: 30d)" },
            json_option,
        },
        .details = &.{
            "Reports only usage recorded by ffx on this machine.",
...
pub const top_level_help_groups = [_]TopLevelHelpGroup{
    ...
    .{ .entries = &.{
        .{ .kind = .login, .usage = "login [codex|grok]", .summary = "Sign in with Codex or Grok" },
        .{ .kind = .logout, .usage = "logout [codex|grok]", .summary = "Sign out of a Codex or Grok session" },
        .{ .kind = .provider, .usage = "provider <provider-id>", .summary = "Choose the active model provider" },
        .{ .kind = .models, .usage = "models" },
    } },
    .{ .entries = &.{
        .{ .kind = .setup, .usage = "setup", .summary = "Configure a provider API key" },
        .{ .kind = .teams, .usage = "teams", .summary = "Choose a Gateway team" },
        .{ .kind = .credits, .usage = "credits|balance", .summary = "Show Gateway credits" },
    } },
    .{ .entries = &.{
    ...
    .{ .entries = &.{
        ...
        .{ .kind = .upgrade, .usage = "upgrade", .summary = "Upgrade ffx on the selected release channel" },
        .{ .kind = .acp, .usage = "acp" },
...
pub const top_level_flags = [_]TopLevelFlag{
    ...
    .{
        .usage = "-v, --version",
        .description = "Print the ffx version and exit",
    },
};

pub const top_level_examples = [_]TopLevelExample{
    .{ .command = "ffx", .description = "Start a fresh interactive session" },
    .{ .command = "ffx ask \"Explain the changes in this repository\"", .description = "Run one request and exit" },
    .{ .command = "ffx session resume last", .description = "Continue the latest session for this workspace" },
    .{ .command = "ffx status --json", .description = "Inspect the current configuration as JSON" },
};
...
pub const top_level_resources = [_]TopLevelResource{
    .{ .label = "Learn more about ffx:", .value = "https://ffx.sh/docs", .link = true },
    .{ .label = "Report a problem:", .value = "run `/feedback` inside ffx" },
};
...
pub const top_level_registry = TopLevelRegistry{
    ...
    .interactive_hint = "ffx starts an interactive session by default. Use `ffx ask` to run one noninteractive request.",
...
pub const slash_specs = [_]SlashSpec{
    ...
    .{ .kind = .provider, .command = "/provider", .aliases = &.{ "/setup", "/connect-provider", "/switch-provider" }, .help_entry = "/provider (/setup, /connect-provider, /switch-provider)", .completion_description = "choose, connect, or switch the model provider", .presentation_category = .account, .has_args = true },
    ...
    .{ .kind = .model, .command = "/model", .aliases = &.{"/models"}, .help_entry = "/model (/models) <id-or-query>", .completion_description = "choose what model and reasoning effort to use", .presentation_category = .model, .has_args = true, .accepts_payload = true },
    .{ .kind = .permissions, .command = "/permissions", .help_entry = "/permissions [ask|auto|full-access|reset]", .completion_description = "choose what ffx is allowed to do", .presentation_category = .security, .show_in_welcome = true, .has_args = true, .accepts_payload = true },
    ...
    .{ .kind = .feedback, .command = "/feedback", .help_entry = "/feedback", .completion_description = "open the ffx feedback form", .presentation_category = .product, .show_in_welcome = true },
    ...
    .{ .kind = .version, .command = "/version", .help_entry = "/version", .completion_description = "show the ffx version", .presentation_category = .general },
...
test "built-in slash registry resolves primary commands and aliases" {
    ...
    try std.testing.expectEqual(command_specs.SlashKind.model, slash_registry.lookup("/models").?.kind);
