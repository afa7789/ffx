...
//! ignored so the integration cannot block or terminate an ffx session.
...
const source = "custom:ffx";
const agent_name = "ffx";
...
pub const Client = struct {
    ...
    /// Report ffx's session id so herdr can associate the pane with a resumable
    ...
    /// Name the pane/agent so herdr lists ffx even while idle. Best-effort;
    ...
    /// Remove ffx from the pane on exit: drop the reported agent and clear the
    /// label/name. Without this the pane keeps showing "ffx" after ffx is gone.
...
test "shouldEnable honors FFX_HERDR opt-out" {
...
test "report_agent serializes a single newline-delimited json line" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"7\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:ffx\",\"agent\":\"ffx\",\"state\":\"working\"," ++
...
test "report_agent omits custom_status when null" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"1\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:ffx\",\"agent\":\"ffx\",\"state\":\"idle\"}}\n",
        out.written(),
    );
}
...
test "report_agent escapes pane id" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"2\",\"method\":\"pane.report_agent\",\"params\":{\"pane_id\":\"pane\\\"x\"," ++
            "\"source\":\"custom:ffx\",\"agent\":\"ffx\",\"state\":\"blocked\"}}\n",
        out.written(),
    );
}
...
test "pane.rename labels the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeRename(&out.writer, 4, "pane.rename", "pane_id", "w1:p1", "label", "ffx");
    try std.testing.expectEqualStrings(
        "{\"id\":\"4\",\"method\":\"pane.rename\",\"params\":{\"pane_id\":\"w1:p1\",\"label\":\"ffx\"}}\n",
        out.written(),
    );
}
...
test "agent.rename names the agent" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeRename(&out.writer, 5, "agent.rename", "target", "w1:p1", "name", "ffx");
    try std.testing.expectEqualStrings(
        "{\"id\":\"5\",\"method\":\"agent.rename\",\"params\":{\"target\":\"w1:p1\",\"name\":\"ffx\"}}\n",
        out.written(),
    );
}
...
test "clear_agent_authority removes ffx from the pane" {
    var out: std.Io.Writer.Allocating = .init(std.testing.allocator);
    defer out.deinit();
    try writeClearAuthority(&out.writer, 7, "w1:p1");
    try std.testing.expectEqualStrings(
        "{\"id\":\"7\",\"method\":\"pane.clear_agent_authority\",\"params\":{\"pane_id\":\"w1:p1\",\"source\":\"custom:ffx\"}}\n",
        out.written(),
    );
}
...
test "report_agent_session serializes session identity" {
    ...
    try std.testing.expectEqualStrings(
        "{\"id\":\"3\",\"method\":\"pane.report_agent_session\",\"params\":{\"pane_id\":\"w1:p1\"," ++
            "\"source\":\"custom:ffx\",\"agent\":\"ffx\",\"agent_session_id\":\"session-42\"}}\n",
        out.written(),
    );
}
