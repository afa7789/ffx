...
fn issuerMismatchResult(
    ...
    try std.json.Stringify.value(returned.bytes, .{}, &out.writer);
    try out.writer.writeAll("} to this server's entry in ~/.ffx/mcp.json and retry.");
...
test "saving MCP config replaces the file durably" {
    ...
    try writeTempFile(&tmp, "home/.ffx/mcp.json", original);
    const path = try tmpDirPath(alloc, tmp.dir, "home/.ffx/mcp.json");
    defer alloc.free(path);

    var fx_dir = try tmp.dir.openDir(io_mod.getIo(), "home/.ffx", .{ .iterate = true });
...
test "MCP config diagnostic treats nonblocking profile states as clear" {
    ...
    try writeTempFile(&tmp, "home/.ffx/mcp.json", "{\"mcp\":{}}");
    {
        const test_home = try TestHome.install(alloc, home_path);
        defer test_home.deinit();
...
test "MCP config diagnostic preserves the startup parser error" {
    ...
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeTempFile(&tmp, "home/.ffx/mcp.json", "{invalid json");
    const home_path = try tmpDirPath(alloc, tmp.dir, "home");
    defer alloc.free(home_path);
...
test "built-in MCP runtime loads disabled configured servers without spawning" {
    ...
    try writeTempFile(&tmp, "home/.ffx/mcp.json",
...
test "saving MCP config refuses a symlinked target" {
    ...
    try tmp.dir.symLink(io_mod.getIo(), external_path, "home/.ffx/mcp.json", .{ .is_directory = false });

    const path = try std.fs.path.join(alloc, &.{ std.fs.path.dirname(external_path).?, ".ffx", "mcp.json" });
    defer alloc.free(path);
...
test "adding an MCP server creates the profile directory privately" {
    ...
    const dir_stat = try tmp.dir.statFile(io_mod.getIo(), "home/.ffx", .{ .follow_symlinks = false });
    try std.testing.expectEqual(@as(u32, 0o700), dir_stat.permissions.toMode() & 0o777);
    const file_stat = try tmp.dir.statFile(io_mod.getIo(), "home/.ffx/mcp.json", .{ .follow_symlinks = false });
...
test "MCP auth requires explicit browser confirmation and logout stays non-secret" {
    ...
    try expectLine(
        mismatch,
        "MCP authentication for 'mismatch' was rejected: expected issuer \"https://signin.auth.plain.com/\" but metadata returned \"https://signin.auth.plain.com\". Add \"oauth\":{\"issuer\":\"https://signin.auth.plain.com\"} to this server's entry in ~/.ffx/mcp.json and retry.",
        false,
    );
...
test "remote config keeps credential references and OAuth policy without secrets" {
    ...
    try std.testing.expectEqualStrings("ffx-client", auth.client_id.?);
...
test "profile config rejects a mixed set containing invalid client metadata URLs" {
    const json =
        \\{"mcp":{"loopback":{"type":"http","url":"https://api.example.com/mcp","oauth":{"client_metadata_url":"http://127.0.0.1:4321/client.json"}},"pathless":{"type":"http","url":"https://api.example.com/mcp","oauth":{"client_metadata_url":"https://client.example"}},"root":{"type":"http","url":"https://api.example.com/mcp","oauth":{"client_metadata_url":"https://client.example/"}},"valid":{"type":"http","url":"https://api.example.com/mcp","oauth":{"client_metadata_url":"https://client.example/ffx.json"}}}}
    ;
    try std.testing.expectError(
