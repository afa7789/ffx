/// Direct provider registry. Mirrors sst/opencode's `Provider × Protocol` model
/// at a much smaller scale: ffx picks one provider per session, the provider
/// declares which env var holds the API key and what its default base URL is.
///
/// Adding a new provider:
///   1. Append a `Provider` literal here.
///   2. Run `zig build` to surface the change in the registry tests.
///   3. The `/provider <id>` slash command picks it up automatically; the
///      direct-provider credential source resolves it without any gateway hop.
...
pub fn resolveBaseUrl(allocator: std.mem.Allocator, provider: Provider) ![]u8 {
    ...
    if (std.posix.getenv(env_var)) |override| {
