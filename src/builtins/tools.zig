...
const memory_description =
    "Save, list, or clear durable user preferences for future ffx sessions. When to use: the user explicitly asks to remember, forget, save, or recall a preference. When NOT to use: store task notes, secrets, project facts, temporary context, or anything the user did not ask to persist.";
...
const install_skill_description =
    "Install a reusable skill from a supported source into ffx managed skill storage. When to use: the user asks to install a skill or pastes a skills install command. When NOT to use: no installation is required, install packages, fetch unrelated repos, or modify project code.";
...
test "built-in subagent owns product metadata schema and callbacks" {
    ...
    try std.testing.expect(std.mem.find(u8, subagent.description, "ordinary ffx child sessions") != null);
