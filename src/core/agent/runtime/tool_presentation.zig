...
fn commandArtifactHandle(
    ...
    if (!std.mem.startsWith(u8, handle, "ffx-command-") or
...
test "command completion publishes its combined artifact handle" {
    ...
    try finishExecutedToolStatus(
        ...
        .{
            ...
            .command_result_json =
            \\{"kind":"foreground","output_file":"/tmp/ffx-command-combined.log"}
            ,
        },
        ...
        .{
            ...
            .command_output_replay = .{ .available = .{
                .handle = "ffx-command-replay-terminal.bin",
                .framed_bytes = 123,
            } },
        },
        null,
    ...
    switch (capture.events.items[0]) {
        .terminal => |terminal| {
            ...
            try std.testing.expectEqualStrings(
                "ffx-command-replay-terminal.bin",
                descriptor.handle,
            );
