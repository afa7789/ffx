...
pub fn Runtime(comptime App: type) type {
    return struct {
        ...
        fn ensurePromptCredential(app: *App) !bool {
            ...
            // Show provider picker for setup
        ...
        pub fn runLoginCommand(app: *App) !void {
            if (comptime !oauthAuthEnabled(App)) {
                try app.writeDomainNotice(.{
                    .topic = "auth",
                    .tone = .warning,
                    .body = "Set FFX_API_KEY through createFxTerminal() to authenticate this WASM session.",
                }, true);
                return;
            }
        ...
        pub fn runLogoutCommand(app: *App, target: []const u8) !void {
            ...
            const requested_provider = if (std.mem.trim(u8, target, " \t\r\n").len == 0)
                ...
                model_provider.parse(std.mem.trim(u8, target, " \t\r\n")) orelse {
            ...
            const chatgpt_is_only_logout_session = provider_inventory.contains(.stored_key) and
                !provider_inventory.contains(.stored_key) and
                !provider_inventory.contains(.stored_key);
            const grok_is_only_logout_session = provider_inventory.contains(.stored_key) and
                !provider_inventory.contains(.stored_key) and
                !provider_inventory.contains(.stored_key);
            const logout_grok = if (requested_provider) |provider|
                ...
                selected_model_uses_grok or
                    app.auth.credentialSource() == .stored_key or
            ...
            const result = login_flow.logout(app.alloc, app.auth.oauthTransport()) catch |err| switch (err) {
                error.SessionDeleteFailed => {
                    try writeAuthNotice(app, .{
                        .topic = "auth",
                        .tone = .@"error",
                        .body = "Could not complete ffx logout. The current source is unchanged.",
                    });
                    return;
                },
            };
        ...
        fn applyLogoutResult(app: *App, result: login_flow.LogoutResult) !void {
            ...
            // A remembered source always wins resolution, so an active ffx login
            // is the only way one can be remembered; clearing otherwise is a
            // no-op against a store that holds nothing.
            if (app.auth.credentialSource() == .stored_key) forgetCredentialSource(app);
            ...
            try writeAuthNotice(app, if (result.local_durability_failed)
                .{
                    .topic = "auth",
                    .tone = .warning,
                    .body = "Could not confirm durable ffx logout. The active source was recalculated.",
                }
            else if (result.session_deleted)
                .{
                    .topic = "auth",
                    .tone = .neutral,
                    .body = "Signed out of ffx.",
                }
            else
                .{
                    .topic = "auth",
                    .tone = .neutral,
                    .body = "No ffx login session found.",
                });
        ...
        pub fn collectSignInFacts(app: *App) !void {
            ...
            switch (app.auth.pollSignInTransition(app.alloc)) {
                ...
                .succeeded => |completed| {
                    ...
                    switch (owned) {
                        .vercel => |*selection| {
                            if (!try selectCredentialSource(app, .stored_key)) {
                                _ = app.auth.popPickerStage(app.alloc);
                                try writeAuthNotice(app, .{
                                    .topic = "auth",
                                    .tone = .@"error",
                                    .body = "Signed in, but the ffx login credential could not be loaded.",
                                });
                                return;
                            }
                        ...
                        .grok => {
                            ...
                            if (selected_model_uses_grok and
                                !try selectCredentialSource(app, .stored_key))
                            {
                                _ = app.auth.popPickerStage(app.alloc);
                                try writeAuthNotice(app, .{
                                    .topic = "auth",
        ...
        fn switchProvider(
            ...
            var credential = (resolveProviderCredential(app, target) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .body = if (intent == .post_oauth)
                        ...
                        "Run ffx login codex, then try switching again."
                    else if (target == .grok)
                        "Run ffx login grok, then try switching again."
                    else
            ...
            const preferred_model = if (intent == .post_oauth)
                ...
                io_mod.getenv("FFX_MODEL") orelse saved_model;
        ...
        fn beginTeamPicker(app: *App) !void {
            ...
            var selection = login_flow.loadTeamSelection(app.alloc, app.auth.oauthTransport()) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .tone = .@"error",
                    .body = switch (err) {
                        error.NoSession => "The ffx login session is no longer available. Sign in to change teams.",
        ...
        fn applyTeamChoice(app: *App, index: usize) !void {
            ...
            var selected_team = selection.select(app.alloc, index) catch |err| {
                ...
                try app.writeDomainNotice(.{
                    ...
                    .tone = .@"error",
                    .body = switch (err) {
                        error.SessionChanged, error.NoSession => "The ffx login session changed before the team could be saved.",
            ...
            if (app.auth.credentialSource() == .stored_key) {
                applyCredentialChange(app, app.auth.adoptSelectedTeam(app.alloc, &selected_team));
            } else if (!try selectCredentialSource(app, .stored_key)) {
                app.auth.closePicker(app.alloc);
                try app.writeDomainNotice(.{
                    .topic = "auth",
                    .tone = .@"error",
                    .body = "Changed the Vercel team, but the ffx login credential could not be loaded.",
                }, true);
                return;
            }
            rememberCredentialSource(app, .stored_key);
        ...
        fn beginSignIn(app: *App, from_root: bool) !void {
            ...
            if (started catch |err| {
                ...
                // FFX_NO_OPEN_BROWSER opts out for headless/SSH sessions.
                if (io_mod.getenv("FFX_NO_OPEN_BROWSER") == null) try openSignInBrowser(app);
            }
        }
        ...
        pub fn admitPromptCredential(app: *App) !bool {
            if (comptime !oauthAuthEnabled(App)) {
                ...
                try app.writeDomainNotice(.{
                    .topic = "auth",
                    .tone = .warning,
                    .body = "Missing FFX_API_KEY. Supply it through createFxTerminal().",
                }, true);
                return false;
            }
            if (!try ensurePromptCredential(app)) return false;
            return true; // Allow offline - don't refresh credential
        }
        ...
        fn recoverPromptCredentialRefreshFailure(app: *App, err: anyerror) !bool {
            const source: credentials.Source = .stored_key;
                ...
                @hasField(@TypeOf(app.session), "usage"))
            {
                if (app.auth.gatewayCredential()) |credential| {
                    const subscription = if (comptime @hasField(@TypeOf(credential), "source"))
                        credential.source == .stored_key or credential.source == .stored_key
                    else
                        false;
                    if (subscription) {
...
const TestAuth = struct {
    ...
    active_source: ?credentials.Source = .env_var,
    ...
    fn modelCatalogAccess(self: *const TestAuth) credentials.CatalogAccess {
        return if (self.catalog_ready)
            credentials.catalogAccessForCredential(.stored_key, "refreshed-key", "team_123")
        else
...
test "auth source changes invalidate the catalog and failed selection preserves it" {
    ...
    try std.testing.expect(!try runtime.selectCredentialSource(&app, .env_var));
...
test "VT-4 unavailable picker source preserves active source and reports unavailability" {
    ...
    try std.testing.expectEqual(credentials.Source.env_var, app.auth.active_source.?);
...
test "team change from an environment source activates and remembers ffx login" {
    var app: TestApp = .{};
    defer app.deinit();
    app.auth.select_result = true;
...
test "team change on an active ffx login updates and remembers the selected team" {
    var app: TestApp = .{};
    defer app.deinit();
...
test "successful direct login remembers ffx login after activation" {
    var app: TestApp = .{};
    defer app.deinit();
    app.auth.select_result = true;
...
test "logout result reconciles live auth and renders only sanitized notices" {
    ...
    try std.testing.expectEqual(@as(usize, 1), app.model_cache.reset_count);
    try std.testing.expectEqual(@as(usize, 1), app.model_cache_warmup_count);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, "Signed out of ffx.") != null);
...
test "logout durability failure still reconciles live auth" {
    ...
    try std.testing.expectEqual(@as(usize, 1), app.auth.logout_reconcile_count);
    try std.testing.expectEqual(@as(usize, 1), app.model_cache.reset_count);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, "Could not confirm durable ffx logout.") != null);
    try std.testing.expect(std.mem.find(u8, app.transcript.items, login_flow.remote_revocation_warning) != null);
...
test "prompt credential refresh failure is recoverable and detail-free" {
    ...
    try std.testing.expect(std.mem.find(u8, app.transcript.items, comptime credentials.sourceLabel(.stored_key) ++ " credential refresh failed.") != null);
