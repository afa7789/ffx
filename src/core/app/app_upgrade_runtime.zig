        ...
        pub fn applyReadyUpgradeWithDeps(
            ...
            app.prepareResumeHandoffForUpgrade() catch |err| {
                const notice = try std.fmt.allocPrint(
                    app.alloc,
                    "upgrade paused because this conversation is not safely resumable: {s}; run `ffx doctor` for recovery guidance",
                    .{@errorName(err)},
                );
            ...
            const executable_path = deps.current_executable_path(
                ...
                const notice = try std.fmt.allocPrint(
                    app.alloc,
                    "upgrade installed, but the executable path could not be resolved: {s}; restart ffx manually",
                    .{@errorName(err)},
                );
                defer app.alloc.free(notice);
                try writeUpgradeNotice(app, .@"error", notice);
            ...
            app.requestUpgradeRelaunch(executable_path) catch |err| {
                const notice = try std.fmt.allocPrint(
                    app.alloc,
                    "upgrade installed, but relaunch could not be prepared: {s}; restart ffx manually",
                    .{@errorName(err)},
                );
                defer app.alloc.free(notice);
                try writeUpgradeNotice(app, .@"error", notice);
...
fn currentExecutablePathForTest(
    ...
    const executable_path = "/tmp/ffx-upgraded";
