...
pub const ProviderSet = std.EnumSet(model_provider.ProviderId);

pub const ProviderPickerMode = enum {
    connect,
    switch_existing,
};

pub const SignInProvider = enum {
    vercel,
    codex,
    grok,
};

/// The provider-specific OAuth modules implement the protocol, while the
/// picker consumes this small common acquisition contract. Adding another
/// OAuth provider only requires an adapter entry here; picker state and input
/// handling remain provider agnostic.
pub const OAuthAdapter = struct {
    provider: SignInProvider,
    provider_id: model_provider.ProviderId,
    start: *const fn (*login_flow.SignInRuntime, Allocator, oauth_transport.Provider) anyerror!bool,
    source_exists: *const fn (Allocator) anyerror!bool,
};
...
fn startVercelOAuth(
    ...
    return runtime.start(alloc, transport);
}
...
fn startCodexOAuth(
    ...
    return chatgpt_oauth.startSignIn(runtime, alloc, transport);
}
...
fn startGrokOAuth(
    ...
    return grok_oauth.startSignIn(runtime, alloc, transport);
}

fn vercelSourceExists(alloc: Allocator) !bool {
    var mutation = (try oauth_session.beginExistingMutation()) orelse return false;
    defer mutation.deinit();
    _ = alloc;
    return true;
}

const oauth_adapters = [_]OAuthAdapter{
    .{ .provider = .vercel, .provider_id = .gateway, .start = startVercelOAuth, .source_exists = vercelSourceExists },
    .{ .provider = .codex, .provider_id = .codex, .start = startCodexOAuth, .source_exists = chatgpt_oauth.sourceExists },
    .{ .provider = .grok, .provider_id = .grok, .start = startGrokOAuth, .source_exists = grok_oauth.sourceExists },
};

pub fn oauthAdapter(provider: SignInProvider) ?*const OAuthAdapter {
    for (&oauth_adapters) |*adapter| {
        if (adapter.provider == provider) return adapter;
    }
    return null;
}

const provider_picker_order = [_]model_provider.ProviderId{
    .opencode_go,
    .openrouter,
    .ppq,
    .codex,
...
pub const PickerView = struct {
    ...
    available_providers: ProviderSet = .empty,
    provider_picker_mode: ProviderPickerMode = .switch_existing,
    provider_picker_target: model_provider.ProviderId = .gateway,
    include_skip: bool,
    ...
    sign_in_provider: SignInProvider = .vercel,
    api_key_provider: model_provider.ProviderId = .gateway,
    ...
    pub fn choiceCount(self: PickerView) usize {
        return switch (self.stage) {
            .root => if (comptime host_target.is_wasm) 2 else 4,
            .provider => providerChoiceCount(self.available_providers, self.provider_picker_mode),
            .method => 2,
            .sign_in, .api_key => 0,
            .change_team => blk: {
                var count: usize = 0;
    ...
    pub fn choiceDescription(self: PickerView, choice: Choice) []const u8 {
        return switch (choice) {
            .provider => |provider| if (provider == self.active_provider)
                "current"
            else if (self.available_providers.contains(provider))
                "connected"
            else
                "",
            ...
            .action => |action| switch (action) {
                .login => "",
                .chatgpt_login => if (self.available_providers.contains(.codex)) "connected" else "",
                .grok_login => if (self.available_providers.contains(.grok)) "connected" else "",
                .api_key => "",
                .setup => "",
    ...
    pub fn choiceEnabled(_: PickerView, choice: Choice) bool {
        return switch (choice) {
            .action => |action| (action != .change_team) and
                ...
                (action != .api_key or !host_target.is_wasm),
...
fn providerChoiceAt(available: ProviderSet, mode: ProviderPickerMode, index: usize) ?model_provider.ProviderId {
    var visible: usize = 0;
    for (provider_picker_order) |provider| {
        if (mode == .connect and provider == .gateway) continue;
        if (mode == .switch_existing and !available.contains(provider)) continue;
        if (visible == index) return provider;
        visible += 1;
    }
    return null;
}
...
pub fn loadStatusSnapshotForProvider(
    ...
    const chatgpt_connected = false;
    const grok_connected = false;
    // Simple credential resolution: try stored key first, then env var.
    const resolution = credentials.resolveForProvider(
        alloc,
        secret_store,
        if (provider) |active| @tagName(active) else null,
    ...
    const gateway_connected = resolved_source != null;
...
pub const Runtime = struct {
    ...
    provider_inventory: ProviderSet = .empty,
    stored_key_status: credentials.StoredKeyReadStatus = .not_attempted,
    ...
    api_key_provider: model_provider.ProviderId = .gateway,
    api_key_save_provider: model_provider.ProviderId = .gateway,
    ...
    fn gatewayCredentialAt(self: *const Self, _: i64) ?GatewayCredential {
    ...
    pub fn modelCatalogAccess(self: *const Self) credentials.CatalogAccess {
        if (self.credential_refresh_failure_source) |source| {
            return .{ .public_only = .{ .authenticated_credential_rejected = source } };
        }
        return credentials.catalogAccessAt(self.selected_credential);
    }
    ...
    fn credentialNeedsRefreshAt(self: *const Self, _: i64) bool {
        _ = self;
        return false;
    }
    ...
    fn statusSnapshotAt(self: *const Self, _: i64) StatusSnapshot {
        const gateway_connected = self.source_inventory.contains(.env_var) or
            self.source_inventory.contains(.env_var) or
            self.source_inventory.contains(.stored_key) or
            self.source_inventory.contains(.stored_key);
        const chatgpt_connected = self.source_inventory.contains(.stored_key);
        const grok_connected = self.source_inventory.contains(.stored_key);
    ...
    pub fn refreshChatGptSourceInventory(self: *Self, _: Allocator) !void {
        _ = self;
    }

    pub fn refreshGrokSourceInventory(self: *Self, _: Allocator) !void {
        _ = self;
    }

    pub fn setProviderInventory(self: *Self, providers: ProviderSet) void {
        self.provider_inventory = providers;
    }

    pub fn providerPickerMode(self: *const Self) ProviderPickerMode {
        return self.provider_picker_mode;
    }
    ...
    pub fn pickerView(self: *const Self) PickerView {
        return .{
            ...
            .available_providers = self.provider_inventory,
            .provider_picker_mode = self.provider_picker_mode,
            .provider_picker_target = self.provider_picker_target,
            ...
            .sign_in_provider = self.sign_in_provider,
    ...
    pub fn openProviderPicker(
        ...
        self.openProviderPickerWithMode(alloc, active_provider, .switch_existing);
    }

    pub fn openConnectProviderPicker(
        self: *Self,
        alloc: Allocator,
        active_provider: model_provider.ProviderId,
    ) void {
        self.openProviderPickerWithMode(alloc, active_provider, .connect);
    }

    fn openProviderPickerWithMode(
        self: *Self,
        alloc: Allocator,
        active_provider: model_provider.ProviderId,
        ...
        self.provider_picker_mode = mode;
        self.picker_selection = if (mode == .switch_existing and self.provider_inventory.contains(active_provider))
            .{ .provider = active_provider }
        else
            self.pickerView().choiceAt(0);
    }

    pub fn openCodexMethodPicker(self: *Self, alloc: Allocator) void {
        self.exitSignInStage(alloc);
        self.exitApiKeyStage(alloc, .screen_replacement);
        self.picker_active = true;
        self.picker_stage = .method;
        self.provider_picker_target = .codex;
        self.picker_selection = self.pickerView().choiceAt(0);
    }
    ...
    pub fn openProviderApiKeyPicker(self: *Self, alloc: Allocator, provider: model_provider.ProviderId) void {
    ...
    pub fn openSignInPicker(self: *Self, alloc: Allocator) !bool {
        return self.openSignInPickerWithParent(alloc, false, .vercel);
    }

    pub fn openSignInPickerFromRoot(self: *Self, alloc: Allocator) !bool {
        return self.openSignInPickerWithParent(alloc, true, .vercel);
    }
    ...
    pub fn openChatGptSignInPickerFromRoot(self: *Self, alloc: Allocator) !bool {
        ...
        return self.openSignInPickerWithParent(alloc, true, .codex);
    }

    pub fn openChatGptSignInPickerForProviderSwitch(self: *Self, alloc: Allocator) !bool {
        if (comptime host_target.is_wasm) return error.ChatGptOAuthUnavailable;
        return self.openSignInPickerWithParent(alloc, false, .codex);
    }
    ...
    pub fn openGrokSignInPickerFromRoot(self: *Self, alloc: Allocator) !bool {
        ...
        return self.openSignInPickerWithParent(alloc, true, .grok);
    }

    pub fn openGrokSignInPickerForProviderSwitch(self: *Self, alloc: Allocator) !bool {
        if (comptime host_target.is_wasm) return error.GrokOAuthUnavailable;
        return self.openSignInPickerWithParent(alloc, false, .grok);
    }
    ...
    fn openSignInPickerWithParent(
        ...
        self.exitSignInStage(alloc);
        const adapter = oauthAdapter(provider) orelse return false;
        const started = try (adapter.start)(&self.sign_in_flow, alloc, self.oauth_transport);
        ...
        self.picker_selection = null;
        self.sign_in_provider = provider;
    ...
    pub fn beginApiKeySave(self: *Self, alloc: Allocator) ApiKeySaveStart {
        if (!self.api_key_save.isSaving()) self.api_key_save_provider = self.api_key_provider;
        return if (self.api_key_provider == .gateway)
            self.beginApiKeySaveWithDeps(alloc, .{
                .ctx = self,
            ...
            self.beginApiKeySaveWithDeps(alloc, .{
                .ctx = self,
                .store = storeRuntimeProviderSecret,
                .loader = loadRuntimeProviderCredential,
    ...
    pub fn takeApiKeySaveResult(self: *Self, alloc: Allocator) ?ApiKeySaveResult {
        ...
        return switch (outcome) {
            ...
            .loaded => |*credential| blk: {
                ...
                const changed = if (self.api_key_save_provider == .gateway)
                    self.adoptCredential(alloc, &owned)
                else
                    false;
                break :blk .{ .saved = .{
                    .changed = changed,
                    .provider = self.api_key_save_provider,
                } };
            },
        };
    }
    ...
    pub fn popPickerStage(self: *Self, alloc: Allocator) bool {
        ...
        if (stage == .provider) {
            self.picker_stage = .root;
            self.picker_selection = .{ .action = if (self.provider_picker_mode == .connect) .setup else .switch_provider };
            return true;
        }
        if (stage == .method) {
            self.openProviderPickerWithMode(alloc, self.provider_picker_active, .connect);
            self.picker_selection = .{ .provider = self.provider_picker_target };
            return true;
        }
        ...
        self.picker_selection = .{ .action = switch (stage) {
            .root, .provider, .method => unreachable,
            .sign_in => switch (self.sign_in_provider) {
                .vercel => .login,
                .codex => .chatgpt_login,
                .grok => .grok_login,
            },
    ...
    pub fn takePickerChoice(self: *Self, alloc: Allocator) ?Choice {
        ...
        switch (self.picker_stage) {
            ...
            .method => switch (selected) {
                .action => |action| switch (action) {
                    .chatgpt_login, .api_key => self.closePicker(alloc),
                    else => unreachable,
                },
    ...
    pub fn adoptCredential(self: *Self, alloc: Allocator, credential: *credentials.Credential) bool {
        const changed = if (self.selected_credential) |selected|
            selected.source != credential.source or
                ...
                !optionalBytesEqual(selected.gatewayTeam(), credential.gatewayTeam())
        else
            true;
    ...
    pub fn adoptSelectedTeam(self: *Self, alloc: Allocator, selected_team: *login_flow.SelectedTeam) bool {
        ...
        if (credential.source != .stored_key) return false;
    ...
    pub fn selectForProvider(
        ...
        _ = provider;
        return self.reselectByPrecedenceWithDeps(
            alloc,
            self,
    ...
    pub fn reconcileAfterChatGptLogout(self: *Self, _: Allocator) !bool {
        _ = self;
        return false;
    }

    pub fn reconcileAfterGrokLogout(self: *Self, _: Allocator) !bool {
        _ = self;
        return false;
    }
    ...
    fn reconcileAfterFxLoginLogoutWithDeps(
        ...
        const login_was_active = self.credentialSource() == .stored_key;
        if (login_was_active) {
            if (self.selected_credential) |*credential| credential.deinit(alloc);
            self.selected_credential = null;
            self.credential_refresh_failure_source = null;
...
fn loadRuntimeCredentialSource(raw: ?*anyopaque, alloc: Allocator, source: credentials.Source) !?credentials.Credential {
    ...
    return credentials.loadSource(alloc, self.secret_store, source);
}
...
fn storeRuntimeProviderSecret(raw: ?*anyopaque, alloc: Allocator, value: []const u8) !void {
    const self: *Runtime = @ptrCast(@alignCast(raw.?));
    return credentials.storeDirectProviderKey(
        alloc,
        model_provider.registryId(self.api_key_save_provider),
        value,
    );
}

fn loadRuntimeProviderCredential(
    raw: ?*anyopaque,
    alloc: Allocator,
    _: credentials.Source,
) !?credentials.Credential {
    const self: *Runtime = @ptrCast(@alignCast(raw.?));
    const provider_id = model_provider.registryId(self.api_key_save_provider);
    const env_var = switch (self.api_key_save_provider) {
        ...
        .gateway, .codex, .grok => return null,
    };
    return credentials.resolveDirectProvider(alloc, self.secret_store, provider_id, env_var);
}
...
test "auth runtime token refresher ignores non-refreshable credential sources" {
    for ([_]credentials.Source{ .env_var, .env_var, .stored_key }) |source| {
...
test "auth failure snapshot keeps refresh failures distinct from HTTP rejection" {
    ...
    const message = try snapshot.renderText(std.testing.allocator);
    defer std.testing.allocator.free(message);
    try std.testing.expectEqualStrings(comptime credentials.sourceLabel(.stored_key) ++ " credential refresh failed", message);

    const json = try snapshot.renderJson(std.testing.allocator);
    defer std.testing.allocator.free(json);
    var parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator, json, .{});
    defer parsed.deinit();
    try std.testing.expectEqualStrings(credentials.sourceLabel(.stored_key), parsed.value.object.get("source").?.string);
    ...
    try std.testing.expect(FailureSnapshot.fromHttp(.forbidden, .stored_key) == null);
...
test "catalog access records a refresh failure until another credential is adopted" {
    ...
    var login = try makeTestCredential(alloc, "login-token", .stored_key, null, null);
    defer login.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &login);
    runtime.recordCredentialRefreshFailure(.stored_key);

    const failed = runtime.modelCatalogAccess();
    try std.testing.expectEqual(credentials.CatalogPublicOnlyReason.authenticated_credential_rejected, failed.publicOnlyReason().?);

    var api_key = try makeTestCredential(alloc, "api-key", .env_var, null, null);
...
test "auth runtime adopts credential ownership and prefers team id" {
    ...
    var different_source = try makeTestCredential(alloc, "token-a", .stored_key, null, "team_123");
    defer different_source.deinit(alloc);

    // Ownership equality compares the effective Gateway team, so a team slug
    // that resolves to the same team is not a change.
    try std.testing.expect(!runtime.adoptCredential(alloc, &different_source));
    try std.testing.expectEqual(credentials.Source.stored_key, runtime.credentialSource().?);

    var unchanged = try makeTestCredential(alloc, "token-a", .stored_key, null, "team_123");
...
test "auth runtime never expires a credential" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);

    var credential = try makeTestCredential(alloc, "stable-token", .stored_key, "team_123", "vercel-labs");
    defer credential.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &credential);
    ...
    // Credentials never expire, so expired is always false and refreshable is always false.
    ...
    var refreshed = try makeTestCredential(alloc, "stable-token", .stored_key, "team_123", "vercel-labs");
    defer refreshed.deinit(alloc);
    try std.testing.expect(!runtime.adoptCredential(alloc, &refreshed));
    try std.testing.expect(!runtime.credentialNeedsRefreshAt(40_000));
    try std.testing.expectEqualStrings("stable-token", runtime.gatewayCredentialAt(40_000).?.api_key);
}
...
test "auth runtime view preserves missing and loaded states" {
    ...
    runtime.source_inventory.insert(.stored_key);
    var credential = try makeTestCredential(alloc, "token", .stored_key, "team_123", null);
    defer credential.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &credential);
    const loaded = runtime.view();
    try std.testing.expectEqual(credentials.Source.stored_key, loaded.active_source.?);
    ...
    try std.testing.expect(!loaded.available_inactive_sources.contains(.stored_key));
}
...
test "auth status snapshot labels every credential source without exposing tokens" {
    ...
    for (sources) |source| {
        ...
        try std.testing.expectEqual(false, snapshot.refreshable());
...
test "auth status snapshot distinguishes an absent store from an unreadable one" {
    ...
    const resolved = StatusSnapshot{ .active_source = .stored_key, .stored_key_status = .unavailable };
    try std.testing.expect(resolved.missingHelp(.cli) == null);
}

test "auth status snapshot renders an expired marker alongside the static refresh flag" {
    const alloc = std.testing.allocator;

    const fresh = StatusSnapshot{ .active_source = .stored_key, .team = "vercel-labs" };
    const fresh_detail = try fresh.formatDoctorDetail(alloc);
    defer alloc.free(fresh_detail);
    try std.testing.expectEqualStrings(
        comptime credentials.sourceLabel(.stored_key) ++ " is configured; refreshable=false; team=vercel-labs",
        fresh_detail,
    );

    const stale = StatusSnapshot{ .active_source = .stored_key, .team = "vercel-labs", .expired = true };
    const stale_detail = try stale.formatDoctorDetail(alloc);
    defer alloc.free(stale_detail);
    try std.testing.expectEqualStrings(
        comptime credentials.sourceLabel(.stored_key) ++ " is configured; session expired; refreshable=false; team=vercel-labs",
        stale_detail,
    );

    // The expired marker is additional state and never flips the static flag.
...
test "auth runtime detects only credential sources that exist" {
    ...
    var runtime: Runtime = .{};
    var probe = Probe{ .existing = SourceSet.initMany(&.{ .env_var, .stored_key }) };
    ...
    try std.testing.expect(inventory.contains(.env_var));
...
test "auth runtime explicitly selects the requested credential source" {
    ...
    var startup = try makeTestCredential(alloc, "startup-token", .env_var, null, null);
    defer startup.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &startup);

    try std.testing.expect((try runtime.selectSourceWithLoader(alloc, .stored_key, null, Loader.load)).?);
    try std.testing.expectEqual(credentials.Source.stored_key, runtime.credentialSource().?);
    try std.testing.expectEqualStrings("stored_key", runtime.apiKey().?);
}

test "auth runtime failed selection preserves the active credential" {
    const Loader = struct {
        fail_stored_key: bool,

        fn load(ctx: ?*anyopaque, alloc: Allocator, source: credentials.Source) !?credentials.Credential {
            const self: *@This() = @ptrCast(@alignCast(ctx.?));
            if (source == .stored_key) {
                if (self.fail_stored_key) return error.SourceReadFailed;
                return null;
            }
            return try makeTestCredential(alloc, @tagName(source), source, null, null);
        }
    ...
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    var active = try makeTestCredential(alloc, "active-token", .env_var, null, null);
    defer active.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &active);

    var loader = Loader{ .fail_stored_key = false };
    try std.testing.expect((try runtime.selectSourceWithLoader(alloc, .stored_key, &loader, Loader.load)) == null);
    try std.testing.expectEqual(credentials.Source.env_var, runtime.credentialSource().?);
    try std.testing.expectEqualStrings("active-token", runtime.apiKey().?);

    loader.fail_stored_key = true;
...
test "logout replaces an active stored key with the next available source" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    runtime.skipOnboarding();

    var active = try makeTestCredential(alloc, "ffx-token", .stored_key, null, null);
    defer active.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &active);

    var fixture = LogoutFixture{
        .existing = SourceSet.initMany(&.{ .env_var, .stored_key }),
    };
    ...
    try std.testing.expectEqual(credentials.Source.env_var, runtime.credentialSource().?);
    try std.testing.expectEqualStrings("env_var", runtime.apiKey().?);
    try std.testing.expect(runtime.source_inventory.contains(.env_var));
    try std.testing.expect(runtime.view().onboarding_skipped);
}
...
test "logout preserves an active non-login credential" {
    ...
    var active = try makeTestCredential(alloc, "active-api-key", .env_var, null, null);
    defer active.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &active);
    runtime.source_inventory.insert(.stored_key);

    var fixture = LogoutFixture{ .existing = SourceSet.initOne(.env_var) };
    try std.testing.expect(!try runtime.reconcileAfterFxLoginLogoutWithDeps(
        alloc,
        &fixture,
        LogoutFixture.probe,
...
test "logout reconciliation adopts a newer concurrent stored key" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);

    var active = try makeTestCredential(alloc, "old-ffx-token", .stored_key, null, null);
    defer active.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &active);

    var fixture = LogoutFixture{ .existing = SourceSet.initOne(.stored_key) };
    try std.testing.expect(try runtime.reconcileAfterFxLoginLogoutWithDeps(
        alloc,
        &fixture,
        LogoutFixture.probe,
...
test "auth picker root starts on adding a provider" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);

    runtime.source_inventory = SourceSet.initMany(&.{ .env_var, .stored_key });
    var credential = try makeTestCredential(alloc, "token", .env_var, null, null);
    defer credential.deinit(alloc);
    _ = runtime.adoptCredential(alloc, &credential);
...
test "credential switcher lists only gateway credential sources" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    runtime.source_inventory = SourceSet.initMany(&.{.env_var});
    runtime.openPicker(alloc);
    runtime.openSwitchCredentialPicker(alloc);

    const picker = runtime.pickerView();
    try std.testing.expectEqual(@as(usize, 2), picker.choiceCount());
    try std.testing.expect((Choice{ .source = .env_var }).eql(picker.choiceAt(0).?));
    try std.testing.expect((Choice{ .action = .automatic }).eql(picker.choiceAt(1).?));
}

test "connect provider picker prioritizes OpenCode OpenRouter and PPQ" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    ...
    const picker = runtime.pickerView();
    try std.testing.expectEqual(ProviderPickerMode.connect, picker.provider_picker_mode);
    try std.testing.expect((Choice{ .provider = .opencode_go }).eql(picker.choiceAt(0).?));
...
test "switch provider picker contains only connected providers" {
    ...
    var runtime: Runtime = .{};
    defer runtime.deinit(alloc);
    runtime.setProviderInventory(ProviderSet.initMany(&.{ .opencode_go, .ppq, .codex }));

    runtime.openProviderPicker(alloc, .ppq);
    const picker = runtime.pickerView();
    try std.testing.expectEqual(ProviderPickerMode.switch_existing, picker.provider_picker_mode);
    try std.testing.expectEqual(@as(usize, 3), picker.choiceCount());
    try std.testing.expect((Choice{ .provider = .opencode_go }).eql(picker.choiceAt(0).?));
    try std.testing.expect((Choice{ .provider = .ppq }).eql(picker.choiceAt(1).?));
    try std.testing.expect((Choice{ .provider = .codex }).eql(picker.choiceAt(2).?));
    try std.testing.expect((Choice{ .provider = .ppq }).eql(picker.selected_choice.?));
    try std.testing.expect(picker.choiceAt(3) == null);
}

test "auth picker navigation wraps across the concise setup actions" {
    const alloc = std.testing.allocator;
    var runtime: Runtime = .{};
    runtime.source_inventory = SourceSet.initMany(&.{ .env_var, .stored_key });
    runtime.openPicker(alloc);
    ...
    try std.testing.expect(runtime.movePicker(-1));
    try std.testing.expect((Choice{ .action = .switch_provider }).eql(runtime.pickerView().selected_choice.?));
}
...
test "auth onboarding picker exposes the setup paths" {
    ...
    try std.testing.expect((Choice{ .action = .switch_provider }).eql(picker.choiceAt(3).?));
...
test "api key stage zeroes its allocation on every exit path" {
    const sentinel = "FFX_API_KEY_ZERO_SENTINEL";
...

    {
        ...
        const result: ApiKeySaveResult = switch (outcome) {
            .loaded => .{ .saved = .{ .changed = true, .provider = .gateway } },
            .gateway_refused => .gateway_refused,
            .gateway_unavailable => .gateway_unavailable,
            .store_failed => .store_failed,
            .reload_failed => .reload_failed,
...
test "oauth adapters expose a shared acquisition contract" {
    const codex = oauthAdapter(.codex) orelse return error.TestUnexpectedResult;
    try std.testing.expectEqual(SignInProvider.codex, codex.provider);
    try std.testing.expectEqual(model_provider.ProviderId.codex, codex.provider_id);
    try std.testing.expect(@intFromPtr(codex.start) != 0);
    try std.testing.expect(@intFromPtr(codex.source_exists) != 0);

    const grok = oauthAdapter(.grok) orelse return error.TestUnexpectedResult;
    try std.testing.expectEqual(model_provider.ProviderId.grok, grok.provider_id);
    try std.testing.expect(oauthAdapter(.vercel) != null);
}
