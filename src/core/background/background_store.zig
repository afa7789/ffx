...
test "saved record roundtrip preserves every field" {
    ...
    try std.testing.expectEqualStrings("/tmp/ffx", loaded.cwd);
    try std.testing.expectEqualStrings("/tmp/ffx.log", loaded.log_path);
...
test "schema v2 roundtrip preserves stable identity token and managed log authority" {
    ...
    var record = Record{
        ...
        .log_storage = .{ .managed_session = .{
            .managed_log_name = try alloc.dupe(u8, "ffx-cmd-test.log"),
        } },
        .expect_url = true,
        .started_at_ms = 100,
        .updated_at_ms = 200,
...
test "Record.fromTaskSnapshot deep-copies fields and optional server URL" {
    ...
    try std.testing.expectEqualStrings("/tmp/ffx.log", record.log_path);
