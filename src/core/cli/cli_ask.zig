    ...
    const result = runPromptInternal(alloc, options.prompt, options.permission_override, effective_cfg, .{
        ...
        if (err == error.OneOffSessionNotResumable and !options.json_output) {
            try deps.write_stderr(
                deps.stderr_ctx,
                "ffx ask: subagent child sessions cannot be resumed directly; message the named agent from its parent session\n",
            );
            return 1;
        }
...
fn runPromptInternal(alloc: Allocator, prompt: []const u8, permission_override: ?PermissionMode, cfg: Config, options: RunOptions) !PromptRunResult {
    ...
    if (ctx.mcp) |mcp| {
        ...
        if (project_names.len > 0) {
            try ctx.writeStderr("ffx ask: skipped unapproved project MCP servers: ");
            for (project_names, 0..) |name, index| {
                if (index > 0) try ctx.writeStderr(", ");
                try ctx.writeStderr(name);
            }
            try ctx.writeStderr(". Approve with ffx mcp trust approve <name> before retrying.\n");
        }
        if (options.output_mode.isTerminal()) {
    ...
    switch (attempt) {
        ...
        .failure => |failure| {
            ...
            try message.writer.print(
                "ffx ask: failed to save full access acknowledgment: {s}\n",
...
test "ffx ask publishes a complete refreshed credential before later consumers" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
...
test "ffx ask auto mode applies automatic clear and caution without a prompt" {
...
test "ffx ask text and JSON share the selected auth failure facts" {
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expectEqualStrings(
        "ffx ask: AI_GATEWAY_API_KEY authentication failed · HTTP 401\n",
        stderr_capture.bytes.items,
    );
    var parsed = try std.json.parseFromSlice(std.json.Value, alloc, stdout_capture.bytes.items, .{});
    defer parsed.deinit();
...
test "ffx ask JSON preserves shell action and runtime error identity" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "ffx ask forwards its catalog and deduplicates repeated discovery warnings" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "ffx ask JSON reports the consumed attempt after retry admission failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
