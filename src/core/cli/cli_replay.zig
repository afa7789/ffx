//! `ffx replay <tape>` implementation.
//!
...
fn runWithOutput(alloc: Allocator, args: []const [:0]const u8, output: anytype) !u8 {
    ...
    const file = std.Io.Dir.cwd().openFile(io_mod.getIo(), opts.path, .{}) catch |err| {
        return replyFormattedError(alloc, output, opts.json, @errorName(err), 512, "ffx replay: cannot open {s}: {s}\n", .{ opts.path, @errorName(err) }, "ffx replay: open failed\n");
    };
    var file_mut = file;
    defer file_mut.close(io_mod.getIo());

    const bytes = io_mod.readFileToEnd(alloc, &file_mut, 64 * 1024 * 1024) catch |err| {
        return replyFormattedError(alloc, output, opts.json, @errorName(err), 256, "ffx replay: read failed: {s}\n", .{@errorName(err)}, "ffx replay: read failed\n");
    };
    defer alloc.free(bytes);

    var parser = record_tape.Parser.init(bytes) catch |err| {
        return replyFormattedError(alloc, output, opts.json, @errorName(err), 256, "ffx replay: bad tape: {s}\n", .{@errorName(err)}, "ffx replay: bad tape\n");
    };
    ...
    if (opts.frames_dir) |dir| {
        prepareFramesDir(alloc, dir) catch |err| {
            return replyFormattedError(alloc, output, opts.json, @errorName(err), 512, "ffx replay: cannot prepare frames dir {s}: {s}\n", .{ dir, @errorName(err) }, "ffx replay: cannot prepare frames dir\n");
        };
    }
    ...
    while (true) {
        ...
        if (opts.frames_dir) |dir| {
            writeFrameArtifacts(alloc, dir, frame_count, frame, elapsed_ms, grid, markers.items) catch |err| {
                return replyFormattedError(alloc, output, opts.json, @errorName(err), 512, "ffx replay: cannot write frame artifacts to {s}: {s}\n", .{ dir, @errorName(err) }, "ffx replay: cannot write frame artifacts\n");
            };
        }
    }
    ...
    if (ignored_incomplete_tail) {
        try output.writeStderr("ffx replay: ignored incomplete final tape frame\n");
    }

    if (opts.frames_dir) |dir| {
        writeFramesManifest(alloc, dir, parser.header, frame_count, resize_count, stdout_bytes) catch |err| {
            return replyFormattedError(alloc, output, opts.json, @errorName(err), 512, "ffx replay: cannot write frames manifest to {s}: {s}\n", .{ dir, @errorName(err) }, "ffx replay: cannot write frames manifest\n");
        };
    }
    ...
    if (opts.golden_path) |out_path| {
        var out_file = std.Io.Dir.cwd().createFile(io_mod.getIo(), out_path, .{ .truncate = true }) catch |err| {
            return replyFormattedError(alloc, output, opts.json, @errorName(err), 256, "ffx replay: cannot write {s}: {s}\n", .{ out_path, @errorName(err) }, "ffx replay: write failed\n");
        };
...
fn replyParseError(
    ...
    const msg = switch (err) {
        Error.MissingTapePath => "ffx replay: missing tape path\nusage: ffx replay <tape> [--frames] [--json] [--golden <path>] [--frames-dir <path>]\n",
        Error.TooManyArgs => "ffx replay: too many positional arguments\n",
        Error.UnknownFlag => "ffx replay: unknown flag\n",
        Error.MissingGoldenPath => "ffx replay: --golden requires a path\n",
        Error.MissingFramesDirPath => "ffx replay: --frames-dir requires a path\n",
        else => "ffx replay: argument error\n",
    };
...
test "json replay recovers an incomplete final frame through stderr" {
    ...
    try testing.expectEqualStrings("ffx replay: ignored incomplete final tape frame\n", capture.stderr.written());
}
...
test "run missing tape path returns current stderr" {
    ...
    try testing.expect(std.mem.startsWith(u8, capture.stderr.written(), "ffx replay: missing tape path\n"));
}
...
test "json failures use stdout for missing arguments files and malformed tapes" {
    ...
    try testing.expectEqual(
        @as(u8, 1),
        try runCaptured(alloc, &.{ "/definitely/missing/ffx-replay.fxtape", "--json" }, &missing_file),
    );
...
test "run malformed tape returns bad tape stderr" {
    ...
    const exit_code = try runCaptured(alloc, &.{tape_arg}, &capture);
    try testing.expectEqual(@as(u8, 1), exit_code);
    try testing.expect(std.mem.startsWith(u8, capture.stderr.written(), "ffx replay: bad tape"));
}
