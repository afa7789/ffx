...
const model_preferences_mod = @import("model_preferences.zig");
...
pub const Settings = struct {
    ...
    pub fn deinit(self: *Settings, alloc: Allocator) void {
        ...
        for (self.favorite_models.items) |model| alloc.free(model);
        self.favorite_models.deinit(alloc);
...
fn parseProfileOnlyFields(
    ...
    if (root.object.get("provider")) |provider_value| {
        ...
        settings.provider = model_provider.parse(provider_value.string) orelse .custom;
    }
    ...
    if (root.object.get("favorite_models")) |favorites| {
        if (favorites != .array) return error.InvalidFavoriteModels;
        for (favorites.array.items) |favorite| {
            if (favorite != .string) return error.InvalidFavoriteModels;
            settings_store.validateModel(favorite.string) catch return error.InvalidFavoriteModels;
            try settings.favorite_models.append(alloc, try alloc.dupe(u8, favorite.string));
        }
    }
...
fn mergeSettings(target: *Settings, incoming: *Settings, alloc: Allocator) void {
    ...
    for (incoming.favorite_models.items) |model| {
        var already_present = false;
        for (target.favorite_models.items) |existing| {
            if (std.mem.eql(u8, existing, model)) {
        ...
        if (already_present) {
            ...
            target.favorite_models.append(alloc, model) catch unreachable;
        }
    }
    incoming.favorite_models.clearRetainingCapacity();
