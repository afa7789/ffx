...
test "api key patch sets one provider key and clear removes only that provider" {
    const alloc = std.testing.allocator;
    var arena = std.heap.ArenaAllocator.init(alloc);
    defer arena.deinit();
    ...
    var root = try std.json.parseFromSliceLeaky(
        std.json.Value,
        arena.allocator(),
        "{\"api_keys\":{\"deepseek\":\"old\",\"openai\":\"keep\"}}",
        .{},
    );
    var api_keys: std.StringHashMapUnmanaged([]const u8) = .empty;
    try api_keys.put(arena.allocator(), "deepseek", "new");
    var application = try applyUserPatchToRoot(arena.allocator(), &root, .{ .api_keys = api_keys });
    try std.testing.expect(application.changed);
    try std.testing.expectEqualStrings("new", root.object.get("api_keys").?.object.get("deepseek").?.string);
    try std.testing.expectEqualStrings("keep", root.object.get("api_keys").?.object.get("openai").?.string);

    application = try applyUserPatchToRoot(arena.allocator(), &root, .{ .clear_api_key = "deepseek" });
    try std.testing.expect(application.changed);
    try std.testing.expect(root.object.get("api_keys").?.object.get("deepseek") == null);
    try std.testing.expectEqualStrings("keep", root.object.get("api_keys").?.object.get("openai").?.string);
...
fn applyUserPatchToRoot(
    ...
    if (patch.api_keys.count() > 0) {
        var api_keys_obj = if (root.object.getPtr("api_keys")) |value| blk: {
            if (value.* != .object) return error.InvalidSettingsFormat;
            break :blk value;
        } else blk: {
            try root.object.put(arena, "api_keys", .{ .object = .empty });
            break :blk root.object.getPtr("api_keys").?;
        };
        var api_it = patch.api_keys.iterator();
        while (api_it.next()) |entry| {
            application.changed = try putString(arena, &api_keys_obj.object, entry.key_ptr.*, entry.value_ptr.*) or application.changed;
        }
    }
    if (patch.clear_api_key) |provider| {
        if (root.object.getPtr("api_keys")) |api_keys_value| {
            if (api_keys_value.* != .object) return error.InvalidSettingsFormat;
            if (api_keys_value.object.contains(provider)) {
                _ = api_keys_value.object.orderedRemove(provider);
                application.changed = true;
            }
        }
    }
...
fn validateKnownSettingsObject(
    ...
    if (object.get("api_keys")) |value| {
        if (value != .object) return error.InvalidSettingsFormat;
        var api_it = value.object.iterator();
        while (api_it.next()) |entry| {
            if (entry.value_ptr.* != .string) return error.InvalidSettingsFormat;
        }
    }
...
test "user patch traces metadata without settings content" {
    ...
    try writeStoreFixture(
        tmp.dir,
        "home/.ffx/settings.json",
        "{\"sentinel_secret\":\"FFX_SETTINGS_SECRET\",\"workspaces\":{}}\n",
    );
    ...
    try std.testing.expect(std.mem.find(u8, trace, "FFX_SETTINGS_SECRET") == null);
    try std.testing.expect(std.mem.find(u8, trace, "FFX_MODEL_SECRET") == null);
