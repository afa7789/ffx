...
const command_artifact_file_prefix = "ffx-command-";
const command_artifact_fallback_dir_name = "ffx-command-output";
...
const foreground_session_replace_failure_prefix = "\x00FFX_FOREGROUND_EXEC_FAILED:";
...
fn foregroundSessionExecutable(scratch: Allocator) ![]const u8 {
    if (comptime builtin.is_test) {
        const path_z = std.c.getenv("FFX_TEST_PRODUCT_EXE") orelse
...
test "target replacement marker prefix remains ordinary stderr" {
    ...
    const result = try executeCommand(.{
        .max_command_output_bytes = 4096,
    }, std.testing.allocator, "printf '\\000FFX_FOREGROUND_EXEC_FAILED:target-data\\n' >&2; exit 125", "/tmp");
    defer std.testing.allocator.free(result.output);
...
test "detached session preserves replacement failure with a zero output budget" {
    ...
    defer scratch_state.deinit();
    const argv = [_][]const u8{"/definitely/missing/ffx-command-target"};
