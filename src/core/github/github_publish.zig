...
test "publish trims padded stdout" {
    const stdout = try std.testing.allocator.dupe(u8, " \nhttps://github.com/vercel-labs/ffx/pull/1\n ");
    const stderr = try std.testing.allocator.dupe(u8, "");
    ...
    try std.testing.expect(result.ok);
    try std.testing.expectEqualStrings("https://github.com/vercel-labs/ffx/pull/1", result.text);
}

test "publish transfers already-trimmed stdout ownership" {
    const stdout = try std.testing.allocator.dupe(u8, "https://github.com/vercel-labs/ffx/issues/2");
    const stderr = try std.testing.allocator.dupe(u8, "");
    ...
    try std.testing.expectEqualStrings("https://github.com/vercel-labs/ffx/issues/2", result.text);
}
