...
pub const StatusSnapshot = struct {
    ...
    pub fn writeJson(self: StatusSnapshot, writer: *std.Io.Writer) !void {
        ...
        try std.json.Stringify.value(self.sandbox, .{}, writer);
...
pub const ModelListSnapshot = struct {
    ...
    fn emptyCatalogProviderName(self: ModelListSnapshot) []const u8 {
        return switch (self.provider) {
            ...
            else => model_provider.label(self.provider),
        };
    }
    ...
    fn catalogExplanation(self: ModelListSnapshot) ?[]const u8 {
        ...
        return switch (reason) {
            .no_credential => "Using the public model catalog; configure a provider API key for the full catalog.",
            .authenticated_credential_rejected => "Your credential was rejected; using the public model catalog.",
        };
    }
};
...
pub const SessionListSnapshot = struct {
    ...
    pub fn renderText(self: SessionListSnapshot, alloc: Allocator) ![]u8 {
        ...
        if (self.has_more) {
            try out.writer.print(
                "[sessions] more saved sessions; continue with `ffx sessions {s}--cursor {s}`\n",
                .{ if (self.all_workspaces) "--all " else "", self.next_cursor orelse "" },
            );
        }
        if (self.skipped_invalid > 0) {
            try out.writer.print(
                "[sessions] warning: skipped {d} unreadable saved session{s}; run `ffx doctor` for recovery guidance\n",
...
pub const UpgradeSnapshot = struct {
    ...
    pub fn renderText(self: UpgradeSnapshot, alloc: Allocator) ![]u8 {
        ...
        switch (self.status) {
            ...
            .up_to_date => {
                if (std.mem.eql(u8, self.channel, "dev") and self.latest_revision.len > 0) {
                    try out.writer.print("ffx dev {s} is already up to date (", .{shortRevision(self.latest_revision)});
                } else {
                    try out.writer.writeAll("ffx is already up to date (");
                }
                try writeVersionWithPrefix(&out.writer, self.latest);
                try out.writer.writeAll(")\n");
            },
...
test "core status snapshot text and json stay stable" {
    const snapshot = StatusSnapshot{
        .model = "alpha",
        .auth_help = "Fx needs access to a model provider. Run ffx login, or ffx setup to use an API key, or set FFX_PROVIDER_API_KEY + FFX_PROVIDER_BASE_URL, or set AI_GATEWAY_API_KEY.",
        .permission_mode = .ask,
        .workspace_root = "/tmp/ffx",
        .history_turns = 3,
    ...
    try std.testing.expectEqualStrings(
        "[status] model=alpha\n[status] update_channel=stable\n[status] build_channel=stable\n[status] auth=missing\n[status] auth_refreshable=false\n[status] auth_help=Fx needs access to a model provider. Run ffx login, or ffx setup to use an API key, or set FFX_PROVIDER_API_KEY + FFX_PROVIDER_BASE_URL, or set AI_GATEWAY_API_KEY.\n[status] permission_mode=ask\n[status] sandbox=none\n[status] workspace=/tmp/ffx\n[status] history_turns=3\n[status] session_permission_grants=1\n[status] agent_step_limit=24\n",
        text,
    );
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"status\",\"model\":\"alpha\",\"update_channel\":\"stable\",\"build_channel\":\"stable\",\"build_revision\":\"\",\"auth\":\"missing\",\"auth_refreshable\":false,\"auth_help\":\"Fx needs access to a model provider. Run ffx login, or ffx setup to use an API key, or set FFX_PROVIDER_API_KEY + FFX_PROVIDER_BASE_URL, or set AI_GATEWAY_API_KEY.\",\"permission_mode\":\"ask\",\"sandbox\":\"none\",\"workspace\":\"/tmp/ffx\",\"history_turns\":3,\"session_permission_grants\":1,\"agent_step_limit\":24}",
        json,
    );
}
...
test "core status snapshot includes selected team when present" {
    ...
    defer std.testing.allocator.free(json);
    try std.testing.expectEqualStrings(
        "{\"kind\":\"status\",\"model\":\"alpha\",\"update_channel\":\"stable\",\"build_channel\":\"stable\",\"build_revision\":\"\",\"auth\":\"" ++ comptime credentials.sourceLabel(.stored_key) ++ "\",\"auth_refreshable\":false,\"team\":\"example-team\",\"permission_mode\":\"ask\",\"sandbox\":\"none\",\"workspace\":\"/tmp/ffx\",\"history_turns\":0,\"session_permission_grants\":0,\"agent_step_limit\":24}",
        json,
    );
}
...
test "model list explains public-only and rejected-credential catalogs" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .body = "1 available\n - alpha\nUsing the public model catalog; configure a provider API key for the full catalog.",
        },
        .{
            .snapshot = rejected,
            .text = "[models] 1 available\n - alpha\n[models] Your credential was rejected; using the public model catalog.\n",
            .body = "1 available\n - alpha\nYour credential was rejected; using the public model catalog.",
        },
        .{
    ...
    try std.testing.expect(std.mem.find(u8, quiet_text, "provider API key") == null);
    const quiet_body = try shown.renderInteractiveBody(alloc);
    defer alloc.free(quiet_body);
    try std.testing.expect(std.mem.find(u8, quiet_body, "provider API key") == null);
}
...
test "core session list snapshot text and json stay stable" {
    ...
    try std.testing.expectEqualStrings(
        "[sessions] 1 saved\n - Session title\n   id=abc | 3 turns | Spanish | updated 1970-01-01 00:00:00.002 UTC\n" ++
            "[sessions] more saved sessions; continue with `ffx sessions --cursor v1:2:abc`\n",
        paged_text,
    );
...
test "core empty session detail snapshot text and json stay stable" {
    const detail = session_store.ReadOnlyDetail{
        ...
        .state = .{
            .id = @constCast("sess-empty"),
            .origin_workspace_root = @constCast("/tmp/ffx"),
            .workspace_root = @constCast("/tmp/ffx"),
            .created_at_ms = 1,
            .updated_at_ms = 2,
            .conversation_language = types.ConversationLanguage.literal("en"),
...
test "core session detail JSON includes assistant execution memory" {
    ...
    var results = [_]types.PersistedToolResult{.{
        ...
        .command_output_replay = .{ .available = .{
            .handle = "ffx-command-replay-private-sentinel.bin",
            .framed_bytes = 77,
        } },
    ...
    try std.testing.expect(std.mem.find(u8, json, "ffx-command-replay-private-sentinel.bin") == null);
}
...
test "core doctor snapshot text and json stay stable" {
    ...
    defer std.testing.allocator.free(json);
    try std.testing.expectEqualStrings(
        "{\"kind\":\"doctor\",\"ok_count\":1,\"warn_count\":1,\"fail_count\":0,\"workspace\":\"/tmp/ffx\",\"model\":\"alpha\",\"auth\":\"" ++ comptime credentials.sourceLabel(.env_var) ++ "\",\"auth_refreshable\":false,\"permission_mode\":\"ask\",\"agent_step_limit\":24,\"checks\":[{\"name\":\"auth\",\"status\":\"ok\",\"detail\":\"AI_GATEWAY_API_KEY is configured\"},{\"name\":\"gh\",\"status\":\"warn\",\"detail\":\"GitHub CLI not found in PATH\"}]}",
        json,
    );
}
...
test "core background list and detail snapshots preserve persisted fields" {
    const records = [_]background_store.Record{
        .{
            ...
            .cwd = @constCast("/tmp/ffx"),
            .log_path = @constCast("/tmp/ffx.log"),
            .expect_url = false,
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"background\",\"count\":1,\"records\":[{\"id\":7,\"started_at_ms\":1,\"updated_at_ms\":2,\"pid\":\"100\",\"command\":\"npm run dev\",\"cwd\":\"/tmp/ffx\",\"log_path\":\"/tmp/ffx.log\",\"state\":\"running\",\"server_url\":\"http://localhost:3000\",\"diagnostic\":null}]}",
        list_json,
    );
    ...
    try std.testing.expectEqualStrings(
        "{\"kind\":\"background_detail\",\"id\":7,\"started_at_ms\":1,\"updated_at_ms\":2,\"pid\":\"100\",\"command\":\"npm run dev\",\"cwd\":\"/tmp/ffx\",\"log_path\":\"/tmp/ffx.log\",\"state\":\"running\",\"expect_url\":false,\"server_url\":\"http://localhost:3000\",\"diagnostic\":null,\"exit_code\":null}",
        detail_json,
    );
}
...
test "core upgrade snapshot renders errors and statuses" {
    ...
    try std.testing.expectEqualStrings("ffx is already up to date (v0.2.10)\n", up_to_date_text);
