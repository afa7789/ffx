...
test "direct executor reaps partial spawn and output-limit process groups" {
    ...
    const missing_argv = [_][]const u8{"/definitely/missing/ffx-direct-second-stage"};
    const partial_stages = [_]command_effect.DirectStage{
        ...
        .{
            .executable = "/definitely/missing/ffx-direct-second-stage",
            .argv = &missing_argv,
            .environment_profile = .basic_read_only,
        },
    };
...
test "direct executor fails closed for missing executable" {
    const missing_argv = [_][]const u8{"/definitely/missing/ffx-direct-command"};
    const missing_stages = [_]command_effect.DirectStage{.{
        .executable = "/definitely/missing/ffx-direct-command",
        .argv = &missing_argv,
        .environment_profile = .basic_read_only,
    }};
    try std.testing.expectError(
