...
pub const yolo_warning_text = "YOLO enabled: ffx permission checks disabled";
...
test "configured wildcard command allows only static command grammar" {
    ...
    try std.testing.expectEqual(RuleDecision.none, ruleDecisionForPermissionPattern(rules, "bash", "printf safe && touch /tmp/ffx-marker", .none));
    try std.testing.expectEqual(RuleDecision.none, ruleDecisionForPermissionPattern(rules, "bash", "printf \"$(touch /tmp/ffx-marker)\"", .none));
}
...
test "configured command rules require exact matching outside static grammar" {
    ...
    try std.testing.expectEqual(RuleDecision.none, ruleDecisionForPermissionPattern(exact_rules, "bash", "printf \"$(touch /tmp/ffx-marker)\"", .none));
...
test "configured command deny and ask retain generic wildcard matching" {
    ...
    try std.testing.expectEqual(RuleDecision.deny, ruleDecisionForPermissionPattern(rules, "bash", "printf safe && touch /tmp/ffx-marker", .none));
    try std.testing.expectEqual(RuleDecision.ask, ruleDecisionForPermissionPattern(rules, "custom", "printf \"$(touch /tmp/ffx-marker)\"", .none));
}
