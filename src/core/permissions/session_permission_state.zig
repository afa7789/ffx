...
fn parseV1CommandIdentity(canonical: []const u8) !V1CommandIdentity {
    var offset: usize = 0;
    if (!std.mem.eql(u8, try readIdentityField(canonical, &offset), "ffx-permission-state-v1")) {
        return error.InvalidPermissionIdentity;
    }
...
pub fn commandKeyV2(
    ...
    errdefer out.deinit();
    try writeIdentityField(&out.writer, "ffx-permission-state-v2");
...
test "v1 command migration is deny dominant stable and idempotent" {
    ...
    try appendFixtureRule(
        ...
        1,
        .command,
        try fixtureIdentity(alloc, &.{ "ffx-permission-state-v1", command, cwd, foreground, none, target_os, restricted }),
        "none allow",
        .allow,
    );
    try appendFixtureRule(
        ...
        2,
        .command,
        try fixtureIdentity(alloc, &.{ "ffx-permission-state-v1", command, cwd, foreground, macos, target_os, restricted }),
        "macos deny",
        .deny,
    );
    try appendFixtureRule(
        ...
        4,
        .command,
        try fixtureIdentity(alloc, &.{ "ffx-permission-state-v1", "git diff", cwd, foreground, macos, target_os, restricted }),
        "older deny",
        .deny,
    );
    try appendFixtureRule(
        ...
        5,
        .command,
        try fixtureIdentity(alloc, &.{ "ffx-permission-state-v1", "git diff", cwd, foreground, none, target_os, restricted }),
        "newer deny",
        .deny,
    );
    try appendFixtureRule(
        ...
        6,
        .command,
        try fixtureIdentity(alloc, &.{ "ffx-permission-state-v1", "pwd", cwd, foreground, none, target_os, restricted }),
        "eligible allow",
        .allow,
    );
    try appendFixtureRule(
        ...
        7,
        .command,
        try fixtureIdentity(alloc, &.{ "ffx-permission-state-v1", "ps", cwd, foreground, macos, target_os, restricted }),
        "ineligible allow",
        .allow,
    );
    ...
    const status_key_bytes = try fixtureIdentity(alloc, &.{ "ffx-permission-state-v2", command, cwd, foreground, target_os });
    ...
    const diff_key_bytes = try fixtureIdentity(alloc, &.{ "ffx-permission-state-v2", "git diff", cwd, foreground, target_os });
    ...
    const pwd_key_bytes = try fixtureIdentity(alloc, &.{ "ffx-permission-state-v2", "pwd", cwd, foreground, target_os });
