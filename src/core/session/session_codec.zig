...
test "execution memory codec preserves feedback and reads v1 results without it" {
    ...
    var results = [_]session.PersistedToolResult{.{
        ...
        .command_output_replay = .{ .available = .{
            .handle = "ffx-command-replay-private.bin",
            .framed_bytes = 321,
        } },
    ...
    try std.testing.expect(std.mem.find(u8, encoded.written(), "ffx-command-replay-private.bin") != null);
    ...
    try std.testing.expectEqualStrings("ffx-command-replay-private.bin", descriptor.handle);
...
test "interrupted command presentation is strict and round trips" {
    ...
    const turn: session.HistoryTurn = .{ .interrupted = .{
        ...
        .cancelled_command = .{
            .output_replay = .{ .available = .{
                .handle = "ffx-command-replay.bin",
                .framed_bytes = 42,
            } },
            .command_artifact_handle = "ffx-command.log",
        },
    } };
    var encoded: std.Io.Writer.Allocating = .init(alloc);
    defer encoded.deinit();
    ...
    try std.testing.expectEqualStrings("ffx-command-replay.bin", descriptor.handle);
    try std.testing.expectEqual(@as(usize, 42), descriptor.framed_bytes);
    try std.testing.expectEqualStrings("ffx-command.log", presentation.command_artifact_handle.?);
...
test "durable image snapshots serialize as session-relative locators" {
    ...
    var images = [_]session.ImageAttachment{.{
        ...
        .snapshot_path = @constCast("/tmp/ffx/sessions/session-id/images/image-1-deadbeef.bin"),
    ...
    try std.testing.expect(std.mem.find(
        u8,
        encoded.written(),
        "/tmp/ffx/sessions/session-id/images",
    ) == null);
    try std.testing.expect(std.mem.find(
        u8,
        encoded.written(),
