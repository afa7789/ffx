...
pub const Usage = struct {
    ...
    /// Returns the local session cost. Remote plan balances are intentionally
    /// not represented here because providers expose them inconsistently.
    pub fn totalCost(self: *const Usage) f64 {
        return self.total_cost;
    }

    pub fn totalTokens(self: *const Usage) u64 {
...
pub fn writeSnapshot(writer: *std.Io.Writer, snapshot: Snapshot) !void {
    ...
    // shape. Older ffx binaries reject unknown snapshot fields instead of
