...
pub const CommandHandlers = struct {
    ...
    connect_provider: *const fn (ctx: *anyopaque) anyerror!void,
    switch_provider: *const fn (ctx: *anyopaque) anyerror!void,
...
pub fn route(registry: SlashRegistry, handlers: *const CommandHandlers, cmd: []const u8) !void {
    switch (parse(registry, cmd)) {
        ...
        .connect_provider => try handlers.connect_provider(handlers.ctx),
        .switch_provider => try handlers.switch_provider(handlers.ctx),
...
fn testHandlers(ctx: *TestContext) CommandHandlers {
    return .{
        ...
        .connect_provider = unexpectedNoPayload,
        .switch_provider = unexpectedNoPayload,
