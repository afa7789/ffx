...
test "background headless and interactive launch notices preserve distinct contracts" {
    ...
    const with_url = try formatBackgroundCommandNotice(alloc, 7, .{
        ...
        .command = "npm run dev",
        .cwd = "/tmp/app",
        .log_path = "/tmp/ffx-cmd.log",
        .url = "http://localhost:3000",
        .expect_url = true,
    }, language);
    defer alloc.free(with_url);
    try std.testing.expectEqualStrings("Background #7: server running at http://localhost:3000. Log: /tmp/ffx-cmd.log", with_url);
    ...
    try std.testing.expectEqualStrings("Background #8: server started. Waiting for local URL. Log: /tmp/ffx-cmd.log", waiting);
    ...
    try std.testing.expectEqualStrings("Background #9: command started. Log: /tmp/ffx-cmd.log", command);
    ...
    try std.testing.expectEqualStrings("Command #9 started. Log: /tmp/ffx-cmd.log", interactive.body);
