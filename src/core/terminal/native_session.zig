...
const launcher_mode = "--ffx-internal-terminal-launcher";
const control_mode = "--ffx-internal-terminal-control";
...
pub fn runControlMarker(raw_args: []const [*:0]const u8) !void {
    ...
    const tmux_failure = if (std.mem.startsWith(
        ...
        io_mod.getenv("FFX_TERMINAL_TEST_TMUX_MARKER_FAILURE")
    else
        null;
...
const SupportedRegistry = struct {
    ...
    pub fn cancelAuthorized(
        ...
        if (io_mod.getenv("FFX_TERMINAL_TEST_FAIL_CANCELLATION_OPEN") != null) {
...
fn monitorOperationFailure(operation_name: []const u8, boundary: []const u8) bool {
    const requested = io_mod.getenv("FFX_TERMINAL_TEST_FAIL_MONITOR_OPERATION") orelse
        return false;
...
fn monitorScreenProjectionAllocationFailure() bool {
    const requested = io_mod.getenv(
        "FFX_TERMINAL_TEST_FAIL_MONITOR_SCREEN_PROJECTION_ALLOCATION",
    ) orelse return false;
...
fn monitorOutputScreenProjectionAllocationFailure() bool {
    const requested = io_mod.getenv(
        "FFX_TERMINAL_TEST_FAIL_MONITOR_OUTPUT_SCREEN_PROJECTION_ALLOCATION",
    ) orelse return false;
    return std.mem.eql(u8, requested, "1");
}
...
fn monitorInstallFailure(point: []const u8) bool {
    const requested = io_mod.getenv("FFX_TERMINAL_TEST_FAIL_MONITOR_INSTALL") orelse
        return false;
...
const Session = struct {
    ...
    fn launchTmux(
        ...
        maybeDelayForTest("FFX_TERMINAL_TEST_TMUX_PREPARED_RELEASE_DELAY_MS");
    ...
    fn commitStartupBoundary(self: *Session) void {
        if (self.command != null) {
            maybeDelayForTest("FFX_TERMINAL_TEST_COMMAND_BOUNDARY_DELAY_MS");
        }
        const zio = io_mod.getIo();
        self.mutex.lockUncancelable(zio);
        if (self.command != null) {
...
fn writeAction(
    ...
    if (request.lease == .use) {
        signalTestBarrier("FFX_TERMINAL_TEST_WRITE_BARRIER_PATH");
        maybeDelayForTest("FFX_TERMINAL_TEST_WRITE_DELAY_MS");
    }
...
fn tmuxResizeCheckpointFailure() bool {
    const value = io_mod.getenv(
        "FFX_TERMINAL_TEST_TMUX_RESIZE_CHECKPOINT_FAILURE",
    ) orelse return false;
...
fn tmuxRecoveryFailure(session_id: []const u8, point: []const u8) bool {
    const value = io_mod.getenv(
        "FFX_TERMINAL_TEST_TMUX_RECOVERY_FAILURE",
    ) orelse return false;
    if (!std.mem.eql(u8, value, point)) return false;
    const selected_session = io_mod.getenv(
        "FFX_TERMINAL_TEST_TMUX_RECOVERY_SESSION_ID",
    ) orelse return true;
...
fn failSignalStageForTest(stage: []const u8) bool {
    const requested = io_mod.getenv("FFX_TERMINAL_TEST_FAIL_SIGNAL_STAGE") orelse
        return false;
...
fn closeAction(
    ...
    if (session.durable.record.backend == .tmux and
        io_mod.getenv("FFX_TERMINAL_TEST_INTERRUPT_CLOSE_AFTER_COMMIT") != null)
    {
        owner.mutex.unlock(zio);
        session.write_mutex.unlock(zio);
...
fn tmuxControlMain(session: *Session) void {
    ...
    while (!terminal_seen and !session.backend_detaching.load(.acquire)) {
        ...
        while (session.tmux_lifecycle_index < frames.len) {
            ...
            if (control.kind == .shell_ready) {
                maybeDelayForTest(
                    "FFX_TERMINAL_TEST_TMUX_SHELL_READY_HOST_DELAY_MS",
                );
            }
    ...
    maybeDelayForTest("FFX_TERMINAL_TEST_BACKEND_CLEANUP_DELAY_MS");
    const zio = io_mod.getIo();
    session.write_mutex.lockUncancelable(zio);
...
const TestDurableFixture = struct {
    ...
    fn init(alloc: Allocator) !TestDurableFixture {
        ...
        defer root.close();
        var ffx = try io_mod.openOrCreateVerifiedPrivateDir(&root, ".ffx");
        defer ffx.close();
        var sessions = try io_mod.openOrCreateVerifiedPrivateDir(&ffx, "sessions");
