...
fn proof_verifier(
    ...
    hash.update("ffx.terminal.holder-proof.v2\x00");
...
fn close_initiator_verifier(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ffx.terminal.close-initiator.v2\x00");
...
fn owner_catalog_key(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ffx.terminal.owner-catalog-key.v2\x00");
...
fn owner_catalog_verifier(
    ...
    var hash = std.crypto.hash.sha2.Sha256.init(.{});
    hash.update("ffx.terminal.owner-catalog-proof.v2\x00");
    hash.update(&claim.proof.bytes);
...
/// Reloads authority for the current ffx owner without trusting caller-supplied
...
/// Reloads a proof only through the managed-child capability of the durable ffx
