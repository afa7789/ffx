    ...
    pub fn persistenceUnavailableFailure(arena: Allocator) !ToolExecutionResult {
        const output = try arena.dupe(
            ...
            "background_persistence_required=true\n" ++
                ...
                "message=headless background commands require session persistence so they can be inspected after ffx ask exits. Remove --no-save or restore access to the session store, then retry.\n",
        );
