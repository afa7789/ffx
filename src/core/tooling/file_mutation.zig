...
fn applyWithTestControls(
    ...
    var temp_name_buffer: [".ffx-stage-".len + suffix.len]u8 = undefined;
    @memcpy(temp_name_buffer[0..".ffx-stage-".len], ".ffx-stage-");
    @memcpy(temp_name_buffer[".ffx-stage-".len..], suffix[0..]);
...
fn expectNoStageFiles(root: []const u8) !void {
    ...
    while (try walker.next(std.testing.io)) |entry| {
        try std.testing.expect(std.mem.find(u8, entry.path, ".ffx-stage-") == null);
    }
}
...
test "apply rejects replaced staged sources without deleting foreign replacements" {
    ...
    while (try iterator.next(std.testing.io)) |entry| {
        if (std.mem.startsWith(u8, entry.name, ".ffx-stage-")) {
...
test "apply rejects in-place staged content changes before rename" {
    ...
    var iterator = root_dir.iterate();
    while (try iterator.next(std.testing.io)) |entry| {
        try std.testing.expect(!std.mem.startsWith(u8, entry.name, ".ffx-stage-"));
    }
}
