...
fn upgradeWorkerInner(
    ...
    const tmp_dir = try std.fmt.allocPrint(alloc, "{s}/ffx-upgrade-{s}", .{ tmp_base, rand_hex });
    ...
    const archive_path = try std.fmt.allocPrint(alloc, "{s}/ffx.tar.gz", .{tmp_dir});
    defer alloc.free(archive_path);

    const archive_url = try std.fmt.allocPrint(alloc, "{s}/{s}/ffx-{s}.tar.gz", .{ cdn_base, target.artifactRef(), helpers.platform });
    ...
    const checksum_url = try std.fmt.allocPrint(alloc, "{s}/{s}/ffx-{s}.tar.gz.sha256", .{ cdn_base, target.artifactRef(), helpers.platform });
    ...
    const extracted_bin = try std.fmt.allocPrint(alloc, "{s}/ffx", .{tmp_dir});
...
fn formatProgressStatusLine(buf: []u8, snapshot: ProgressSnapshot, current: []const u8, latest_label: ?[]const u8) ?[]const u8 {
    return switch (snapshot.phase) {
        ...
        .found => blk: {
            ...
            out.print("ffx {s} -> {s}", .{ current, versionLabel(latest) }) catch return out.buffered();
