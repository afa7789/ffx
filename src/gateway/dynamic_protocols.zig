...
pub fn build_responses(
    ...
    _ = try responses.writeTools(writer, alloc, request.tools);
...
/// Caller owns the parsed schemas. Uses the current provider-neutral tool selection.
pub fn tool_schemas(alloc: Allocator, tools: stream_provider.ToolSelection) !std.json.Parsed(std.json.Value) {
    var out: std.Io.Writer.Allocating = .init(alloc);
    defer out.deinit();
    try out.writer.writeAll("{\"tools\":[]");
    _ = try responses.writeTools(&out.writer, alloc, tools);
    try out.writer.writeByte('}');
    return std.json.parseFromSlice(std.json.Value, alloc, out.written(), .{ .allocate = .alloc_always, .duplicate_field_behavior = .use_last });
}
...
pub fn build_anthropic(alloc: Allocator, request: stream_provider.RequestData) ![]u8 {
    try validateModel(request.model);
    ...
    try w.writeByte(']');
    var schemas = try tool_schemas(alloc, request.tools);
    defer schemas.deinit();
    var tools_out: std.Io.Writer.Allocating = .init(alloc);
    defer tools_out.deinit();
    first = true;
    for (schemas.value.object.get("tools").?.array.items) |schema| try write_anthropic_tool(&tools_out.writer, schema, &first);
    if (!first) {
...
pub fn consume_sse(protocol: definition.Protocol, alloc: Allocator, reader: *std.Io.Reader, request: stream_provider.ModelRequest) !types.ModelCompletion {
...
fn apply_anthropic(reducer: *responses.Reducer, alloc: Allocator, raw: []const u8, callbacks: responses.StreamCallbacks, request: stream_provider.ModelRequest) !bool {
...
fn test_request(cancel: *std.atomic.Value(bool), delivery: *stream_provider.DeliveryCertainty, evidence: *stream_provider.AttemptEvidence) stream_provider.ModelRequest {
    const Ignore = struct {
        fn emit(_: *anyopaque, _: stream_provider.Event) void {}
    };
    return .{ .credential = .{ .direct = .{ .secret_bytes = "fixture" } }, .model = "test", .retry_count = 0, .messages = &.{}, .tool_choice = .auto, .provider_options = .{}, .trace_ctx = .{}, .content_capture_limit = null, .delivery = delivery, .attempt_evidence = evidence, .events = .{ .context = cancel, .emit_fn = Ignore.emit }, .cancel_flag = cancel };
}
...
test "dynamic protocols serialize real Responses and Anthropic tool history" {
    ...
    const request: stream_provider.RequestData = .{
        .model = "private-model",
        .instructions = &.{.{ .role = .system, .content = "Be brief" }},
        .messages = &.{ .{ .role = .user, .content = "Read" }, .{ .role = .assistant, .tool_calls = &.{.{ .id = "c1", .name = "read", .arguments_json = "{\"path\":\"a\"}" }} }, .{ .role = .tool, .tool_call_id = "c1", .content = "ok" } },
        .tools = .{ .additional_functions = &.{.{ .name = "read", .description = "Read", .input_schema = .{} }} },
        .tool_choice = .auto,
        .provider_options = .{},
...
test "dynamic protocols Responses events stream text tools and usage" {
    ...
    var result: stream_provider.Result = .{ .completed = .{ .completion = completion, .ownership = .owned } };
