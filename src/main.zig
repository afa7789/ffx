...
const App = struct {
    ...
    pub fn init(alloc: Allocator, launch: *cli_surface.InteractiveLaunch) !Self {
        ...
        const env_disabled = if (io_mod.getenv("FFX_AUTO_UPGRADE")) |val|
    ...
    fn collectThemeFacts(self: *App) !void {
        ...
        // FFX_THEME forces colors via detectTheme; keep owning protocol bytes
...
fn benchmarkEnvPresent(raw_env: RawEnviron) bool {
    if (comptime builtin.link_libc) {
        if (std.c.getenv("FFX_BENCH") != null) return true;
    }
    return rawEnvHas(raw_env, "FFX_BENCH");
}
...
test "raw benchmark preflight matches no-arg FFX_BENCH presence" {
    const no_args = [_][*:0]const u8{"ffx"};
    const help_args = [_][*:0]const u8{ "ffx", "help" };
    const bench_env = [_:null]?[*:0]const u8{"FFX_BENCH=1"};
