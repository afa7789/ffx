...
pub fn call(ctx: tool_dispatch.DispatchContext, erased: tool_dispatch.ToolInput) tool_dispatch.DispatchError!tool_dispatch.ToolResult {
    ...
    const output = runMemory(ctx.allocator, input.action, input.fact) catch |err| switch (err) {
        ...
        error.MemoryStoreMalformed => return .{ .failure = try ctx.allocator.dupe(
            u8,
            "memory store is malformed; ~/.ffx/memories.json was not modified. Repair or remove the file, then retry",
        ) },
        error.MemoryStoreTooLarge => return .{ .failure = try ctx.allocator.dupe(
            u8,
            "memory store exceeds the 1 MiB limit; ~/.ffx/memories.json was not modified. Reduce or remove the file, then retry",
        ) },
        error.MemoryStoreUnreadable => return .{ .failure = try ctx.allocator.dupe(
            u8,
            "memory store could not be read; ~/.ffx/memories.json was not modified. Check the file type and permissions, then retry",
        ) },
...
test "memory clear fails closed when state cannot be deleted" {
    ...
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "home/.ffx/memories.json");
    {
        var survivor = try tmp.dir.createFile(
            io_mod.getIo(),
            "home/.ffx/memories.json/must-survive.txt",
            .{},
        );
...
test "memory corrupt store fails closed and preserves original bytes" {
    ...
    try tmp.dir.writeFile(io_mod.getIo(), .{
        .sub_path = "home/.ffx/memories.json",
        .data = corrupt_store,
    });
    ...
    var file = try tmp.dir.openFile(io_mod.getIo(), "home/.ffx/memories.json", .{});
    defer file.close(io_mod.getIo());
