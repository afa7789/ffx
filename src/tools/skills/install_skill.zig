...
pub fn executeFromSource(alloc: Allocator, skills_dir: []const u8, source: []const u8, filter: ?[]const u8) ![]u8 {
    ...
    try out.writer.print("Installed {d} skill(s) into ffx.\n", .{result.installed.items.len});
...
fn formatNoMatchOutput(alloc: Allocator, source: []const u8) ![]u8 {
    ...
    out.writer.writeAll("No matching skills were installed into ffx from ") catch return error.OutOfMemory;
...
test "install_skill owner installs local skill source" {
    ...
    try expectContains(output, "Installed 1 skill(s) into ffx.");
...
test "install_skill owner reports no matching skills" {
    ...
    const expected = try std.fmt.allocPrint(alloc, "No matching skills were installed into ffx from {s}.", .{repo_root});
...
test "install_skill owner reports status when an unsafe root cannot be installed" {
    ...
    try expectContains(output, "No matching skills were installed into ffx from");
}
