...
/// intentionally absent; Core derives them from the current ffx session.
...
pub fn mapAuthorizedResult(
    ...
    mapped.status_detail = switch (code) {
        ...
        .authority_retired => try alloc.dupe(
            u8,
            "saved terminal authority is from an older ffx version; start a new terminal",
        ),
        else => return result,
    };
    return mapped;
...
test "terminal decoder normalizes gateway stringified start composites" {
    const alloc = std.testing.allocator;
    const args =
        "{\"action\":\"start\",\"cwd\":\"/workspace\",\"backend\":\"native\",\"command\":\"printf SHOULD_NOT_RUN\",\"return_when\":\"{\\\"kind\\\":\\\"started\\\"}\",\"initial_monitors\":\"[{\\\"condition\\\":{\\\"kind\\\":\\\"path_exists\\\",\\\"path\\\":\\\"/private/tmp/ffx-monitor-outside-ready\\\",\\\"check_interval_ms\\\":1000},\\\"notify\\\":{\\\"kind\\\":\\\"on_match\\\"},\\\"lifetime\\\":{\\\"kind\\\":\\\"until_match\\\"}}]\"}";
    const decoded = try decode(.{ .allocator = alloc }, args);
    switch (decoded) {
        .failure => |message| {
            defer alloc.free(message);
        ...
        .input => |input| {
            ...
            try std.testing.expectEqualStrings(
                "/private/tmp/ffx-monitor-outside-ready",
...
test "terminal result mapper adds detail for actionable failures" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .expected_detail = "saved terminal authority is from an older ffx version; start a new terminal",
        },
