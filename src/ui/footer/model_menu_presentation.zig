...
fn loadedCatalogStatusText(state: model_cache_runtime.ModelMenuCatalogState) ?[]const u8 {
    ...
    if (state.access_level == .authenticated) {
        const source = state.source orelse return "Using an authenticated catalog.";
        return switch (source) {
            .env_var => "Catalog: authenticated with an environment variable.",
            .stored_key => "Catalog: authenticated with the stored API key.",
        };
    }
    return null;
}
...
test "model menu status follows provenance and retryable failure precedence" {
    try std.testing.expectEqualStrings(
        "Catalog: authenticated with the stored API key.",
        loadedCatalogStatusText(.{ .access_level = .authenticated, .source = .stored_key }).?,
    );
    ...
    const cases = [_]struct {
        ...
        .{ .state = .{ .public_only_reason = .authenticated_credential_rejected, .private_models_hidden = true }, .expected = "Your credential was rejected; using the public model catalog." },
