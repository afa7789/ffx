...
const picker_test_slash_specs = [_]command_specs.SlashSpec{
    ...
    .{ .kind = .settings, .command = "/settings", .help_entry = "/settings", .completion_description = "configure ffx", .presentation_category = .general },
};
...
test "slash menu keeps matching skill source labels" {
    const skills = [_]skill_runtime.Skill{.{
        .name = "fx-test-strategy",
        .description = "choose focused regression coverage for the affected ffx behavior",
        .path = "/tmp/.codex/skills/fx-test-strategy",
        .source = .global_codex,
    }};
...
test "partially visible auth picker shows a source window without duplicates" {
    ...
    try std.testing.expect(std.mem.find(u8, first_source.items, "ffx login") == null);

    var selected_source = try composeAuthPickerRow(alloc, view, 3, 4, 80);
    defer selected_source.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, selected_source.items, "ffx login") != null);
    ...
    try std.testing.expect(std.mem.find(u8, scrolled_first.items, "ffx login") != null);
