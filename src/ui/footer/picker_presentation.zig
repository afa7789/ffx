...
pub fn authPickerRowCount(view: auth_runtime.PickerView) u16 {
    ...
    if (view.stage == .api_key) return 3;
...
pub noinline fn composeAuthPickerRow(
    ...
    if (view.stage == .sign_in) {
        return composeSignInPickerRow(alloc, view.sign_in, view.sign_in_provider, row_index, width);
    }
    ...
    if (show_header) {
        ...
        const header = switch (view.stage) {
            .root => "   Setup",
            .provider => if (view.provider_picker_mode == .connect) "   Connect provider" else "   Switch provider",
            .method => "   Connect Codex",
            .sign_in => unreachable,
            .api_key => unreachable,
    ...
    const choice = maybe_choice orelse {
        ...
        try row_text.appendClipped(alloc, &row, switch (view.stage) {
            ...
            .method => "     No connection methods available",
            .sign_in => unreachable,
            .api_key => unreachable,
...
const onboarding_note = "   ⚠︎ Note: ffx is experimental and defaults to auto mode.";
const onboarding_note_link = onboarding_note ++ " \x1b]8;id=ffx-onboarding;https://ffx.sh/docs/stability\x1b\\\x1b[4mLearn more\x1b[24m\x1b]8;;\x1b\\";
...
fn composeOnboardingPickerRow(
    ...
    const label = switch (source_row_index) {
        0 => "   Welcome to ffx",
        1 => "",
        2 => "   ffx can access AI models with an account, subscription, or API key.",
        3 => "   Choose a provider below, then connect with an API key.",
        4 => "",
...
fn composeSignInPickerRow(
    ...
    provider: auth_runtime.SignInProvider,
    row_index: u16,
    width: u16,
) !std.ArrayList(u8) {
    var row: std.ArrayList(u8) = .empty;
    ...
    const label = switch (row_index) {
        ...
        ) catch switch (provider) {
            .vercel => "   Open the Vercel authorization page",
            .codex => "   Open the Codex authorization page",
            .grok => "   Open the Grok authorization page",
        },
...
fn composeApiKeyPickerRow(
    ...
    try row.appendSlice(alloc, if (row_index == 1)
        ...
        ui_render.dim_style);
    switch (row_index) {
        0 => try row_text.appendClipped(alloc, &row, "   API key", width),
        1 => {
        ...
        2 => try row_text.appendClipped(alloc, &row, "   Enter to save · Esc to cancel", width),
        else => {},
    }
    try row.appendSlice(alloc, ui_render.reset_style);
    return row;
...
test "slash menu hides metadata for commands and skills" {
    ...
    const skills = [_]skill_runtime.Skill{.{
        .name = "ffx-test-strategy",
        .description = "choose focused regression coverage",
        .path = "/tmp/.codex/skills/ffx-test-strategy",
        .source = .global_codex,
    }};
    const skill_layout = slashMenuLayout(picker_test_slash_registry, "/ffx-test", &skills, 0, 0, 24, 0, 0).?;
    const skill_widths = mixedSlashMenuColumnWidths(picker_test_slash_registry, "/ffx-test", &skills, skill_layout.window, false);
    var skill = try composeSlashMenuOptionRow(std.testing.allocator, picker_test_slash_registry, "/ffx-test", &skills, 0, true, skill_widths, 64, false);
...
test "slash menu keeps matching skill source labels" {
    ...
    const layout = slashMenuLayout(picker_test_slash_registry, "/ffx-test", &skills, 0, 0, 24, 0, 0).?;
    try std.testing.expectEqual(@as(usize, 0), layout.command_count);
    try std.testing.expectEqual(@as(usize, 1), layout.result_count);

    var header = try composeSlashMenuHeaderRow(std.testing.allocator, "/ffx-test", layout, 80);
    defer header.deinit(std.testing.allocator);
    try std.testing.expect(std.mem.find(u8, header.items, "Results 1") != null);

    const column_widths = mixedSlashMenuColumnWidths(picker_test_slash_registry, "/ffx-test", &skills, layout.window, true);
    var row = try composeSlashMenuOptionRow(std.testing.allocator, picker_test_slash_registry, "/ffx-test", &skills, 0, true, column_widths, 64, true);
    defer row.deinit(std.testing.allocator);
    try std.testing.expect(std.mem.find(u8, row.items, "ffx-test-strategy") != null);
...
test "mixed slash completion includes matching skills with source labels" {
    ...
    try std.testing.expect(mixedSlashCompletionCount(picker_test_slash_registry, "/ffx-test", &skills) > 0);
    const skill = nthMixedSlashCompletionSkill(picker_test_slash_registry, "/ffx-test", &skills, 0) orelse return error.TestExpectedEqual;
    try std.testing.expectEqualStrings("ffx-test-strategy", skill.name);

    var row = try composeMixedSlashCompletionOptionRow(alloc, picker_test_slash_registry, "/ffx-test", &skills, 0, true, 1, 8, 50);
    defer row.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, row.items, "ffx-test-strategy") != null);
    try std.testing.expect(std.mem.find(u8, row.items, "codex") != null);
...
test "mixed slash completion uses skill relevance order" {
    const skills = [_]skill_runtime.Skill{
        .{
            ...
            .path = "/tmp/.ffx/skills/metadata-first",
            .source = .global_fx,
        },
        .{
...
test "auth onboarding composes the welcome copy and setup choices" {
    ...
    try std.testing.expect(std.mem.find(u8, screen.items, "Welcome to ffx") != null);
    try std.testing.expect(std.mem.find(u8, screen.items, "ffx can access AI models with an account, subscription, or API key") != null);
    ...
    try std.testing.expect(std.mem.find(u8, body_row.items, "ffx can access AI models") != null);
    ...
    try std.testing.expect(std.mem.find(u8, unselected_row.items, "Switch provider") != null);

    var narrow_note = try composeAuthPickerRow(alloc, view, 12, authPickerRowCount(view), 58);
    defer narrow_note.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, narrow_note.items, "https://ffx.sh/docs/stability") == null);
...
test "auth picker composes only detected credential sources" {
    ...
    const view = auth_runtime.PickerView{
        .active = true,
        .available_sources = auth_runtime.SourceSet.initMany(&.{ .env_var, .stored_key }),
        .selected_choice = .{ .action = .setup },
    ...
    try std.testing.expect(std.mem.find(u8, sign_in.items, "Add an API key") != null);
...
test "compact auth picker keeps the selected hub action visible" {
    ...
    try std.testing.expect(std.mem.find(u8, row.items, "Switch connection") != null);
}
...
test "api key stage renders only a concise bounded mask" {
    const alloc = std.testing.allocator;
    const view = auth_runtime.PickerView{
        .active = true,
        .available_sources = .empty,
    ...
    var field = try composeAuthPickerRow(alloc, view, 1, 3, 80);
    defer field.deinit(alloc);
    try std.testing.expectEqual(@as(usize, 9), std.mem.count(u8, field.items, "•"));
    try std.testing.expect(std.mem.find(u8, field.items, "FFX_API_KEY_RENDER_SENTINEL") == null);

    var hint = try composeAuthPickerRow(alloc, view, 2, 3, 80);
    defer hint.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, hint.items, "Enter to save · Esc to cancel") != null);
}
...
test "partially visible auth picker shows a source window without duplicates" {
    ...
    const view = auth_runtime.PickerView{
        ...
        .selected_choice = .{ .source = .env_var },
        .active_source = .stored_key,
        .include_skip = false,
        .stage = .switch_credential,
    };

    var header = try composeAuthPickerRow(alloc, view, 0, 3, 80);
    defer header.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, header.items, "Use this credential") != null);

    var first_source = try composeAuthPickerRow(alloc, view, 1, 3, 80);
    defer first_source.deinit(alloc);
    try std.testing.expect(std.mem.find(u8, first_source.items, credentials.sourceLabel(.env_var)) != null);
