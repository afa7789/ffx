...
const surface_test_slash_specs = [_]command_specs.SlashSpec{
    ...
    .{ .kind = .feedback, .command = "/feedback", .help_entry = "/feedback", .completion_description = "open the ffx feedback form", .presentation_category = .product },
};
...
test "surface footer keeps the selected auth source visible at minimum height" {
    ...
    for (frame.composed.rows.items) |row| {
        if (std.mem.find(u8, row.text.items, credentials.sourceLabel(.stored_key)) != null) return;
    }
