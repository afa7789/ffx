...
test "active soft-wrapped footer survives every valid tiny height" {
    const input = "FFX_SOFT_START " ++ ("filler " ** 16) ++ "FFX_SOFT_END";
    for ([_]u16{ 5, 6, 7, 8 }) |target_height| {
...
test "welcome logo stays pinned while middle transcript rows overflow" {
    ...
    try expectGridContains(&h, "𝒇𝒇x v");
...
test "startup reservation scrolls to fit first paint without wiping pre-ffx rows" {
    const alloc = std.testing.allocator;
    var h = try Harness.init(alloc, 80, 24, 4);
    defer h.deinit();
...
test "settled resize clears reflowed rows above the launch-owned viewport" {
    ...
    try h.vt.feed("\x1b[9;1HPRE-FFX-ROW");
    try shell_runtime.applyResizeWithLayout(
        &h.shell,
        &h.metrics,
        layoutFor(80, 23, 4),
    ...
    try expectGridNotContains(&h, "PRE-FFX-ROW");
    try expectRowPrefix(&h, 1, "FX01");
    try expectRowPrefix(&h, 11, "FX11");
}
...
test "grow after shrink-overflow restores transcript inside ffx's viewport band" {
    var h = try Harness.init(std.testing.allocator, 80, 30, 4);
    defer h.deinit();
...
test "settled resize clears pre-ffx shell history and reanchors at row one" {
    ...
    try h.shell.initViewport(&h.metrics, 4);
    try h.shell.writeTranscript(h.alloc, &h.metrics, "first ffx line\nsecond ffx line\n", true);
    try h.flush();
    ...
    try expectGridNotContains(&h, "$ ffx");
    try std.testing.expectEqual(@as(u16, 1), h.shell.owned_top_row);
    try std.testing.expectEqual(@as(u16, 1), h.shell.viewport_top_row);
    try expectRowPrefix(&h, 1, "first ffx line");
    try expectRowPrefix(&h, 2, "second ffx line");
}

test "settled width resize clears pre-ffx shell rows and reanchors at row one" {
    const alloc = std.testing.allocator;
    var h = try Harness.init(alloc, 80, 24, 4);
    defer h.deinit();
    ...
    try h.shell.initViewport(&h.metrics, 4);
    try h.shell.writeTranscript(alloc, &h.metrics, "first ffx line\nsecond ffx line\n", true);
    try h.flush();
...
test "large tabbed user turn keeps frame scroll plan aligned" {
    ...
    try h.shell.initViewport(&h.metrics, 12);
    try h.shell.writeTranscript(alloc, &h.metrics, "What would you like ffx to do?\n", true);
    h.frame_redraw = true;
    try renderTestFooter(&h, &input, &approval, &h.frame_redraw);
    try h.flush();
