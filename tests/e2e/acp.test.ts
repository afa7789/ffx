...
async function waitForTerminalHostExit(root: string): Promise<void> {
  const identityPath = join(root, "home", ".ffx", "terminal-host-v7", "host.json");
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        expect(existsSync(join(root.home, ".ffx", "auth.json"))).toBe(false);
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        expect(gateway.requests).toHaveLength(0);
        const imageDir = join(root.home, ".ffx", "sessions", sessionId, "images");
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        expect(statuses.slice(1, -1).every((status: string) => status === "in_progress")).toBe(true);
        expect(client.stderr).toBe("");
      } finally {
        await client?.close();
        gateway.stop();
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    try {
      ...
      await tui.kill();
      tui = null;
      const ids = readdirSync(join(root.home, ".ffx", "sessions"), { withFileTypes: true })
        .filter((entry) => entry.isDirectory()).map((entry) => entry.name);
      expect(ids).toHaveLength(1);
      const eventsPath = join(root.home, ".ffx", "sessions", ids[0]!, "events.jsonl");
