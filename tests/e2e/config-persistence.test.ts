...
function migrationSnapshotPath(home: string, field: string): string {
  const backups = join(home, ".ffx", "backups");
...
describe.skipIf(!tmuxAvailable())("config persistence", () => {
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-input-appearance-persistence-"));
      try {
        ...
        let stored = JSON.parse(readFileSync(join(home, ".ffx", "settings.json"), "utf8"));
        ...
        stored = JSON.parse(readFileSync(join(home, ".ffx", "settings.json"), "utf8"));
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-config-persistence-"));
      ...
      try {
        ...
        writeFileSync(join(workspaceA, ".ffx.json"), projectABytes);
        writeFileSync(join(workspaceB, ".ffx.json"), projectBBytes);
        writeFileSync(
        ...
        expect(readFileSync(join(workspaceA, ".ffx.json"), "utf8")).toBe(projectABytes);
        expect(readFileSync(join(workspaceB, ".ffx.json"), "utf8")).toBe(projectBBytes);
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-config-permission-scopes-"));
      try {
        const home = join(root, "home");
        const workspaceA = join(root, "workspace-a");
        const workspaceB = join(root, "workspace-b");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-config-output-shadow-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        ...
        writeFileSync(join(workspace, ".ffx.json"), projectBytes);
        ...
        expect(readFileSync(join(workspace, ".ffx.json"), "utf8")).toBe(projectBytes);
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-model-picker-dismissal-"));
      const gateway = startFakeGateway([], {
        models: [{
          id: "xai/grok-build-1",
          type: "language",
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-startup-preferences-"));
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-fast-unsupported-"));
      const gateway = startFakeGateway([], {
        models: [{
      ...
      try {
        ...
        session = await TmuxSession.create({
          ...
          env: {
            ...NO_AUTH,
            FFX_PROVIDER_API_KEY: "fake-standard-key",
            HOME: home,
            FFX_E2E_GATEWAY_MODELS_URL: `${gateway.baseUrl}/coding-agent/v1/models`,
          },
          stderrPath,
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-settings-effort-"));
      const gateway = startFakeGateway([], {
        models: [{
          id: "zai/glm-5.2",
          type: "language",
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-anthropic-capabilities-"));
      const gateway = startFakeGateway(
      ...
      try {
        ...
        writeFileSync(
          join(home, ".ffx", "auth.json"),
          JSON.stringify({
            version: 1,
            issuer: "https://vercel.com",
            client_id: "test-client",
            access_token: "fake-ffx-login-token",
            refresh_token: "fake-ffx-login-refresh-token",
        ...
        expect(readFileSync(settingsPath, "utf8")).toBe(initialSettings);
        await session.sendKeys("Enter");
        // Gateway order is preserved after ffx's default sentinel.
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-openai-capabilities-"));
      const gateway = startFakeGateway(
        [
      ...
      try {
        ...
        const gatewayEnv = {
          ...NO_AUTH,
          FFX_PROVIDER_API_KEY: "fake-capability-key",
          HOME: home,
          FFX_GATEWAY_BASE_URL: gateway.baseUrl,
          FFX_GATEWAY_CHAT_URL: gateway.chatUrl,
          FFX_E2E_GATEWAY_MODELS_URL: `${gateway.baseUrl}/coding-agent/v1/models`,
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-fable-capabilities-"));
      const gateway = startFakeGateway([], {
        models: [{
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-model-picker-skill-"));
      const gateway = startFakeGateway([], {
        models: [{
          id: "xai/grok-build-1",
          type: "language",
      ...
      try {
        ...
        const workspace = join(root, "workspace");
        const stderrPath = join(root, "stderr.log");
        const skillRoot = join(home, ".ffx", "skills", "model-helper");
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-gateway-capabilities-"));
      const gateway = startFakeGateway(
        [
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-config-first-write-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const stderrPath = join(root, "stderr.log");
        ...
        expect(statSync(join(home, ".ffx", "history.jsonl")).mode & 0o777).toBe(0o600);
        expect(statSync(join(home, ".ffx", "history.lock")).mode & 0o777).toBe(0o600);
        expect(statSync(join(home, ".ffx", "settings.json")).mode & 0o777).toBe(0o600);
        expect(statSync(join(home, ".ffx", "settings.lock")).mode & 0o777).toBe(0o600);
        expect(readFileSync(stderrPath, "utf8")).toBe("");
      } finally {
        rmSync(root, { recursive: true, force: true });
      }
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-statusline-write-failure-"));
      try {
        const home = join(root, "home");
  ...
  serialTest("config diagnostics preserve invalid, oversized, and unsafe primaries", async () => {
    const root = mkdtempSync(join(tmpdir(), "ffx-config-diagnostics-"));
    try {
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      mkdirSync(join(home, ".ffx"), { recursive: true, mode: 0o700 });
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-config-concurrent-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        mkdirSync(join(home, ".ffx"), { recursive: true, mode: 0o700 });
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-concurrent-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        ...
        [session, secondSession] = await Promise.all([
          ...
          TmuxSession.create({
            cmd: `${FFX_BIN} --add-dir ${launchRoot}`,
            cwd: workspaceRoot,
            env,
            stderrPath: stderrBPath,
          }),
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-capacity-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const added = join(root, "added");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-source-disappears-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-source-moves-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-target-replaced-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-survivor-moves-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-source-retarget-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const first = join(root, "first");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-source-restart-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const first = join(root, "first");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-slash-persistence-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-live-availability-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const shared = join(root, "shared");
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-restored-canonical-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        ...
        try {
          session = await TmuxSession.create({
            ...
            env: {
              HOME: home,
              FFX_PROVIDER_API_KEY: "fake-restored-root-key",
              VERCEL_OIDC_TOKEN: undefined,
              FFX_AUTO_UPGRADE: "0",
              FFX_GATEWAY_BASE_URL: gateway.baseUrl,
              FFX_GATEWAY_CHAT_URL: gateway.chatUrl,
  ...
  serialTest(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-workspace-launch-removal-"));
      try {
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const shared = join(root, "shared");
        ...
        session = await TmuxSession.create({
          cmd: `${FFX_BIN} --add-dir ${sharedRoot}`,
          cwd: workspaceRoot,
          env,
          stderrPath,
        });
