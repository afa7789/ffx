...
const liveTest = test.skipIf(
  !HAS_API_KEY || process.env.FFX_E2E_REAL_API !== "1",
);
...
function createIsolatedRoot() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "ffx-file-paths-e2e-")));
...
function gatewayEnv(
  ...
  return {
    HOME: home,
    FFX_PROVIDER_API_KEY: "fake-file-paths-key",
...
describe("filesystem path handling", () => {
  ...
  liveTest(
    ...
    async () => {
      ...
      const content = `FFX_LIVE_ADDED_ROOT_${Date.now()}`;
      const instructionSentinel = "FFX_LIVE_ADDED_ROOT_INSTRUCTION_MUST_NOT_APPEAR";
        ...
        const result = await runFx(
          ...
          ],
          {
            ...
            env: {
              ...
              FFX_MODEL: process.env.FFX_WORKSPACE_ACCESS_LIVE_MODEL ?? EVAL_MODEL,
            },
            timeoutMs: 120_000,
          },
        );
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot();
      try {
        const homeFile = join(root.home, "ffx-path-fixture.txt");
        const externalFile = join(root.external, "ffx-path-fixture.txt");
        ...
        const cases = [
          {
            id: "read_home_1",
            path: "~/ffx-path-fixture.txt",
            canonical: homeFile,
            content: "HOME_FIXTURE_CONTENT",
          },
          {
            id: "read_relative_1",
            path: "../external/ffx-path-fixture.txt",
  ...
  test(
    "registered typed write and edit use one canonical ffx ask mutation path",
    async () => {
      const root = createIsolatedRoot();
      try {
    ...
    async () => {
      ...
      try {
        const literalWorkspacePath = join(
          root.workspace,
          "~",
          "ffx-path-fixture.txt",
        );
        await runTerminalToolScenario({
          root,
          id: "read_missing_home_1",
          name: "read_file",
          input: { path: "~/ffx-path-fixture.txt" },
          unsetHome: true,
