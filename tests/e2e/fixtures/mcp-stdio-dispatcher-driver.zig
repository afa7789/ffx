...
fn addInteractiveAuthServer(
    ...
    try runtime.addServer(.{
        ...
        .url = try alloc.dupe(u8, server_url),
        .auth = .{
            .client_id = try alloc.dupe(u8, "ffx-mcp-auth-test"),
        },
        .operation_timeout_ms = 10_000,
    });
    runtime.connectAll(.{});
...
fn runRuntimeRecovery(
    ...
    config_env[0] = .{
        .key = try alloc.dupe(u8, "FFX_MCP_MODE"),
        .value = try alloc.dupe(u8, "block_operation_write_once"),
    };
    config_env[1] = .{
        .key = try alloc.dupe(u8, "FFX_MCP_CRASH_MARKER"),
        .value = try alloc.dupe(u8, marker_path),
    };
    config_env[2] = .{
        .key = try alloc.dupe(u8, "FFX_MCP_WIRE_LOG"),
        .value = try alloc.dupe(u8, wire_log_path),
    };
    config_env[3] = .{
        .key = try alloc.dupe(u8, "FFX_MCP_PID_PATH"),
...
fn addRecoveryTestServer(
    ...
    const entries = [_]struct { key: []const u8, value: []const u8 }{
        .{ .key = "FFX_MCP_MODE", .value = mode },
        .{ .key = "FFX_MCP_CRASH_MARKER", .value = marker_path },
        .{ .key = "FFX_MCP_WIRE_LOG", .value = wire_log_path },
        .{ .key = "FFX_MCP_PID_PATH", .value = pid_path },
        .{ .key = "FFX_MCP_RECOVERY_READY_PATH", .value = ready_path },
        .{ .key = "FFX_MCP_RECOVERY_RELEASE_PATH", .value = release_path },
        .{ .key = "FFX_MCP_INITIAL_TOOL_NAME", .value = initial_tool_name },
        .{ .key = "FFX_MCP_RECOVERED_TOOL_NAME", .value = recovered_tool_name },
        .{ .key = "FFX_MCP_RESULT_TEXT", .value = result_text },
    };
