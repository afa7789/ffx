...
function createNotificationRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), "ffx-notifications-")));
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    try {
      ...
      expect(trace).toContain("handler=ffx.sound.turn_end");
...
test(
  "ffx ask keeps redirected stdout JSON and stderr byte-clean with notifications enabled",
  async () => {
    const fixture = createNotificationRoot();
    const gateway = startFakeGateway([
...
test.skipIf(!tmuxAvailable())(
  "ffx ask correlates permission attention and turn-end notifications",
  async () => {
    const fixture = createNotificationRoot();
    ...
    try {
      session = await TmuxSession.create({
        cmd: `${FFX_BIN} ask --no-save "Try the prepared command."`,
        cwd: fixture.workspace,
        env: notificationEnv(fixture.home, gateway, tracePath),
        remainOnExit: true,
      });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    try {
      ...
      expect(waitingTrace).toContain("handler=ffx.sound.attention_required");
      expect(existsSync(marker)).toBe(false);
