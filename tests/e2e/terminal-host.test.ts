...
const INTERNAL_MODE = "--ffx-internal-terminal-host";
...
function makeHome(): string {
  const home = mkdtempSync(join(tmpdir(), "ffx-terminal-host-"));
...
function rememberPrivateTmuxIdentities(resource: PrivateTmuxResource): void {
  ...
  try {
    ...
    for (const name of names.trim().split("\n")) {
      if (name.startsWith("ffx-") && name.length === 35) {
...
function tmuxPeerArtifacts(): string[] {
  return readdirSync("/tmp")
    .filter((name) =>
      name.startsWith("ffx-tmux-capture-") ||
      name.startsWith("ffx-tmux-marker-")
    )
    .sort();
}
...
function tmuxCaptureHelperPids(): number[] {
  ...
  return output
    ...
    .filter((entry): entry is { pid: number; command: string } => entry !== null)
    .filter((entry) =>
      entry.command.includes(FFX_BIN) &&
      entry.command.includes("--ffx-internal-terminal-tmux-capture")
    )
    .map((entry) => entry.pid)
...
async function cleanupPrivateTmuxServer(
  ...
  for (const identity of identities) {
    rmSync(`/tmp/ffx-tmux-capture-${identity}.sock`, { force: true });
    rmSync(`/tmp/ffx-tmux-marker-${identity}.sock`, { force: true });
  }
...
async function runClientFixture(
  ...
  const current = binary === FFX_BIN;
  const executable = current ? buildCurrentClientFixture() : binary;
  const args = current ? [] : ["--ffx-internal-terminal-client-fixture"];
  const child = spawn(executable, args, {
    env: {
      ...process.env,
      HOME: home,
      SHELL: TERMINAL_FIXTURE_SHELL,
      FFX_TERMINAL_HOST_IDLE_MS: String(idleMs),
      ...extraEnv,
    },
...
function buildCurrentClientFixture(): string {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), "ffx-terminal-client-fixture-"));
...
function buildThreadForkFixture(): string {
  ...
  const repoRoot = join(import.meta.dir, "../..");
  const fixtureRoot = mkdtempSync(join(tmpdir(), "ffx-terminal-thread-fork-"));
...
function hostPaths(home: string) {
  const dir = join(home, ".ffx", "terminal-host");
  return {
    dir,
...
function terminalTransportPaths(home: string) {
  ...
  const digest = createHash("sha256")
    .update("ffx.terminal.transport.v1\0")
    .update(home)
    .digest("hex")
    .slice(0, 32);
  ...
  const dir = join(base, `ffx-terminal-${uid}-${digest}`);
  return {
    dir,
    socket: join(dir, "host.sock"),
...
function makeLongHome(endpointBytes = 141): string {
  const root = mkdtempSync(join(tmpdir(), "ffx-terminal-long-home-"));
  const endpointSuffix = join(".ffx", "terminal-host", "host.sock");
...
function startHost(
  ...
  const child = spawn(binary, [INTERNAL_MODE], {
    env: {
      ...
      SHELL: TERMINAL_FIXTURE_SHELL,
      FFX_TERMINAL_HOST_IDLE_MS: String(idleMs),
      FFX_TERMINAL_HOST_PROTOCOL_MIN: String(range.minimum),
      FFX_TERMINAL_HOST_PROTOCOL_CURRENT: String(range.current),
      ...extraEnv,
    },
    stdio: ["pipe", "pipe", "pipe"],
  });
...
function protocolFixtureEnv(
  ...
  return {
    FFX_TERMINAL_HOST_PROTOCOL_MIN: String(fixture.range.minimum),
    FFX_TERMINAL_HOST_PROTOCOL_CURRENT: String(fixture.range.current),
    FFX_TERMINAL_HOST_PROTOCOL_CAPABILITIES: String(fixture.capabilities),
  };
}
...
function ownerCatalogAuthorityForSession(
  ...
  const key = ownerCatalogDigest(
    "ffx.terminal.owner-catalog-key.v2\0",
    ownerPrincipal,
    actor,
  ).toString("hex");
  const verifier = ownerCatalogDigest(
    "ffx.terminal.owner-catalog-proof.v2\0",
    ownerPrincipal,
    actor,
...
test("client reconciles an idle-retiring host before admitting a request", async () => {
  ...
  const retiring = startHost(home, undefined, 50, {
    FFX_TRACE_LOG: trace,
    FFX_TRACE_SCOPES: "terminal_host",
    FFX_TERMINAL_TEST_IDLE_EXIT_DELAY_MS: "3000",
  }, buildCurrentClientFixture());
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    const paths = hostPaths(home);
    const host = startHost(home, undefined, 10_000, {
      FFX_TERMINAL_TEST_COMMAND_BOUNDARY_DELAY_MS: "5000",
    });
    await waitFor(() => existsSync(paths.socket));
    const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
    ...
    for (const fixture of tmuxBoundedPeerFailures) {
      for (let pass = 1; pass <= 2; pass++) {
        ...
        const host = startHost(home, undefined, 2_000, {
          FFX_TRACE_LOG: trace,
          FFX_TRACE_SCOPES: "terminal_host",
          FFX_TERMINAL_TEST_TMUX_DEADLINE_MS: "150",
          ...(fixture.owner === "marker"
            ? { FFX_TERMINAL_TEST_TMUX_MARKER_FAILURE: fixture.point }
            : { FFX_TERMINAL_TEST_TMUX_CAPTURE_FAILURE: fixture.point }),
        });
    ...
    for (const fixture of [
      ...
      for (let pass = 1; pass <= 2; pass++) {
        ...
        const host = startHost(home, undefined, 2_000, {
          FFX_TRACE_LOG: trace,
          FFX_TRACE_SCOPES: "terminal_host",
          FFX_TERMINAL_TEST_TMUX_TCSETPGRP_FAILURE: "1",
          ...(fixture.injectGroupKillFailure
            ? { FFX_TERMINAL_TEST_TMUX_GROUP_KILL_FAILURE: "1" }
            : {}),
        });
        const stderr = streamText(host.stderr);
        await waitFor(() => existsSync(paths.socket));
        ...
        expect(existsSync(`/tmp/ffx-tmux-capture-${record.backend_identity}.sock`))
          .toBe(false);
        expect(existsSync(`/tmp/ffx-tmux-marker-${record.backend_identity}.sock`))
          .toBe(false);
        expect(processFdCount(host.pid!)).toBeLessThanOrEqual(baselineFds + 2);
        connected.client.close();
        expect(await waitForExit(host)).toBe(0);
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    const host = startHost(home, undefined, 250, {
      FFX_TRACE_LOG: trace,
      FFX_TRACE_SCOPES: "terminal_host",
      FFX_TERMINAL_TEST_TMUX_DEADLINE_MS: "2000",
    });
    const stderr = streamText(host.stderr);
    await waitFor(() => existsSync(paths.socket));
    const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
...
static int launcher_process(void) {
  ...
  close(fd);
  const char needle[] = "--ffx-internal-terminal-tmux-launcher";
...
int tcsetpgrp(int fd, pid_t pgrp) {
  ...
  const char *ready = getenv("FFX_E2E_TMUX_DESCENDANT_READY");
  for (int i = 0; ready != NULL && access(ready, F_OK) != 0 && i < 1000; i++) {
    usleep(5000);
  }
  int result = real_tcsetpgrp(fd, pgrp);
  touch_path(getenv("FFX_E2E_TMUX_HANDOFF_ASSIGNED"));
  const char *release = getenv("FFX_E2E_TMUX_HANDOFF_RELEASE");
...
`,
    ...
    const host = startHost(home, undefined, 250, {
      ...
      FFX_E2E_TMUX_DESCENDANT_READY: descendantReady,
      FFX_E2E_TMUX_HANDOFF_ASSIGNED: handoffAssigned,
      FFX_E2E_TMUX_HANDOFF_RELEASE: handoffRelease,
      FFX_TERMINAL_TEST_TMUX_DEADLINE_MS: "2000",
    });
    const stderr = streamText(host.stderr);
    await waitFor(() => existsSync(paths.socket));
    ...
    expect(processLines.some((line) =>
      line.includes("--ffx-internal-terminal-control")
    )).toBe(false);
    expect(existsSync(resumedProof)).toBe(false);
    const record = durableTerminalRecord(home).value as unknown as {
      backend_identity: string;
    ...
    for (const identity of [record.backend_identity, laterRecord.backend_identity]) {
      expect(existsSync(`/tmp/ffx-tmux-capture-${identity}.sock`)).toBe(false);
      expect(existsSync(`/tmp/ffx-tmux-marker-${identity}.sock`)).toBe(false);
    }
    expect(processFdCount(host.pid!)).toBeLessThanOrEqual(hostFds + 2);
    for (const path of [
      wrapper,
...
test.skipIf(!tmuxAvailable())("tmux resize checkpoint failures roll back without losing control", async () => {
  ...
  for (const failurePoint of ["allocation", "storage", "checkpoint"]) {
    ...
    const host = startHost(home, undefined, 30_000, {
      FFX_TERMINAL_TEST_TMUX_RESIZE_CHECKPOINT_FAILURE: failurePoint,
    });
    await waitFor(() => existsSync(paths.socket));
    const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
  ...
  const cases = [
    {
      ...
      lifecycleKind: 1,
      commandless: false,
      env: { FFX_TERMINAL_TEST_TMUX_PREPARED_RELEASE_DELAY_MS: "5000" },
    },
    {
      name: "shell-ready",
      lifecycleKind: 2,
      commandless: true,
      env: { FFX_TERMINAL_TEST_TMUX_SHELL_READY_HOST_DELAY_MS: "5000" },
    },
    {
      name: "command-started",
      lifecycleKind: 3,
      commandless: false,
      env: { FFX_TERMINAL_TEST_COMMAND_BOUNDARY_DELAY_MS: "5000" },
    },
  ];
  ...
  async () => {
    ...
    for (let pass = 1; pass <= 2; pass++) {
      ...
      const firstHost = startHost(home, undefined, 30_000, {
        FFX_TERMINAL_TEST_TMUX_PREPARED_RELEASE_DELAY_MS:
      ...
      const sessionName = `ffx-${record.backend_identity}`;
      ...
      const failed = startHost(home, undefined, 30_000, {
        FFX_TERMINAL_TEST_TMUX_RECOVERY_FAILURE: "release",
      });
...
test.skipIf(!tmuxAvailable())("transient tmux recovery failures preserve the pane and retry state", async () => {
  ...
  for (const failurePoint of [
    ...
    const sessionName = `ffx-${backendIdentity}`;
    const siblingName = `ffx-${siblingIdentity}`;
    const panePid = Number(execFileSync(
      "tmux",
      ["-S", tmuxSocket, "display-message", "-p", "-t", sessionName, "#{pane_pid}"],
      { encoding: "utf8" },
    ...
    const failedRecovery = startHost(home, undefined, 30_000, {
      FFX_TERMINAL_TEST_TMUX_RECOVERY_FAILURE: failurePoint,
      FFX_TERMINAL_TEST_TMUX_RECOVERY_SESSION_ID: sessionId,
    });
    ...
    await waitFor(
      () => !existsSync(`/tmp/ffx-tmux-capture-${backendIdentity}.sock`),
      5_000,
    ).catch(() => {
      throw new Error(`${failurePoint}: target capture socket retained`);
    });
    expect(existsSync(`/tmp/ffx-tmux-capture-${backendIdentity}.sock`), failurePoint)
      .toBe(false);
    ...
    expect(existsSync(`/tmp/ffx-tmux-capture-${backendIdentity}.sock`), failurePoint)
      .toBe(false);
    expect(existsSync(`/tmp/ffx-tmux-capture-${siblingIdentity}.sock`), failurePoint)
      .toBe(false);
    expect(existsSync(`/tmp/ffx-tmux-marker-${backendIdentity}.sock`), failurePoint)
      .toBe(false);
    expect(existsSync(`/tmp/ffx-tmux-marker-${siblingIdentity}.sock`), failurePoint)
      .toBe(false);
  }
}, 180_000);
...
test.skipIf(!tmuxAvailable())("tmux recovery restores the saved workspace scope", async () => {
  ...
  const sessionName = `ffx-${backendIdentity}`;
  const invalidSessionName = `ffx-${invalidBackendIdentity}`;
  const panePid = Number(execFileSync(
    "tmux",
    ["-S", tmuxSocket, "display-message", "-p", "-t", sessionName, "#{pane_pid}"],
    { encoding: "utf8" },
  ...
  expect(existsSync(`/tmp/ffx-tmux-capture-${invalidBackendIdentity}.sock`)).toBe(
    false,
  );
  expect(existsSync(`/tmp/ffx-tmux-marker-${invalidBackendIdentity}.sock`)).toBe(
    false,
  );
  ...
  expect(existsSync(paths.socket)).toBe(false);
  expect(existsSync(paths.identity)).toBe(false);
  expect(existsSync(`/tmp/ffx-tmux-capture-${backendIdentity}.sock`)).toBe(false);
  expect(existsSync(`/tmp/ffx-tmux-marker-${backendIdentity}.sock`)).toBe(false);
  expect(existsSync(`/tmp/ffx-tmux-capture-${invalidBackendIdentity}.sock`)).toBe(
    false,
  );
  expect(existsSync(`/tmp/ffx-tmux-marker-${invalidBackendIdentity}.sock`)).toBe(
...
test.skipIf(!tmuxAvailable())("private tmux teardown owns partial recovery resources", async () => {
  ...
  const failedRecovery = startHost(home, undefined, 30_000, {
    FFX_TERMINAL_TEST_TMUX_RECOVERY_FAILURE: "after-gap",
  });
  ...
  const transportRoot = mkdtempSync(join(tmpdir(), "ffx-terminal-cleanup-root-"));
...
test.each([
  ...
  const host = startHost(home, undefined, 400, {
    FFX_TERMINAL_TEST_FAIL_MONITOR_INSTALL: point,
  });
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
  ...
  const terminalState = join(home, ".ffx", "sessions", TERMINAL_OWNER_SESSION);
...
test("poll path escapes preserve exact failures only for capable peers", async () => {
  for (const testCase of [
    ...
    const home = makeHome();
    const outside = mkdtempSync(join(tmpdir(), "ffx-terminal-monitor-outside-"));
    homes.push(outside);
...
test("custom probe re-canonicalization rejects a post-install symlink swap", async () => {
  ...
  mkdirSync(inside);
  const outside = mkdtempSync(join(tmpdir(), "ffx-terminal-monitor-swap-"));
  homes.push(outside);
...
test("screen projection failure leaves every resize owner unchanged", async () => {
  ...
  const host = startHost(home, undefined, 5_000, {
    FFX_TERMINAL_TEST_FAIL_MONITOR_SCREEN_PROJECTION_ALLOCATION: "1",
  });
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
  const started = await startInteractiveNativeFixture(
...
test("PTY output projection failure skips screen checks while output checks continue", async () => {
  ...
  const paths = hostPaths(home);
  const host = startHost(home, undefined, 300, {
    FFX_TERMINAL_TEST_FAIL_MONITOR_OUTPUT_SCREEN_PROJECTION_ALLOCATION: "1",
  });
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
  const started = await startInteractiveNativeFixture(
...
test("closing a start-ceiling session releases initial monitor ownership", async () => {
  ...
  const paths = hostPaths(home);
  const host = startHost(home, undefined, 300, {
    FFX_TERMINAL_TEST_COMMAND_BOUNDARY_DELAY_MS: "1000",
  });
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
...
test.skipIf(!tmuxAvailable())("simultaneous notifications compose with ordered acknowledgement and mutation", async () => {
  ...
  const host = startHost(home, undefined, 300, {
    FFX_TERMINAL_TEST_ORDER_BARRIER: barrier,
    FFX_TERMINAL_TEST_ORDER_HOLD_CORRELATION: "248",
  });
  await waitFor(() => existsSync(paths.socket));
...
test.each(monitorOperationFailureCases)(
  ...
  async ({ operation, boundary }) => {
    ...
    const paths = hostPaths(home);
    const host = startHost(home, undefined, 300, {
      FFX_TERMINAL_TEST_FAIL_MONITOR_OPERATION: `${operation}:${boundary}`,
    });
    await waitFor(() => existsSync(paths.socket));
    const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
...
test("failed first monitor arming leaves the host idle-owned", async () => {
  ...
  const paths = hostPaths(home);
  const host = startHost(home, undefined, 250, {
    FFX_TERMINAL_TEST_FAIL_MONITOR_OPERATION: "add:arming",
  });
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
  const started = await startInteractiveNativeFixture(
...
test("revoke and close quiesce writes already queued under stale authority", async () => {
  ...
  const host = startHost(home, undefined, 10_000, {
    FFX_TERMINAL_TEST_WRITE_DELAY_MS: "180",
    FFX_TERMINAL_TEST_WRITE_BARRIER_PATH: writeBarrier,
  });
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
  let correlation = 220;
...
test("Bash and zsh preserve trusted normal startup and controlled clean startup", async () => {
  ...
  const paths = hostPaths(home);
  const host = startHost(home, undefined, 10_000, {
    FFX_TERMINAL_TEST_COMMAND_BOUNDARY_DELAY_MS: "2500",
  });
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
  let correlation = 200;
...
test("force close reports incomplete refresh descendant and shell delivery", async () => {
  ...
  for (const [index, stage] of stages.entries()) {
    ...
    const paths = hostPaths(home);
    const host = startHost(home, undefined, 200, {
      FFX_TERMINAL_TEST_FAIL_SIGNAL_STAGE: stage,
    });
    await waitFor(() => existsSync(paths.socket));
    const control = await handshake(paths.socket, { minimum: 4, current: 5 });
    let shellPid = 0;
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    expect(sessionNames).toEqual([`ffx-${validIdentity}`]);
    expect(existsSync(`/tmp/ffx-tmux-capture-${invalidIdentity}.sock`)).toBe(
      false,
    );
    expect(existsSync(`/tmp/ffx-tmux-marker-${invalidIdentity}.sock`)).toBe(
      false,
    );
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    const paths = hostPaths(home);
    const firstHost = startHost(home, undefined, 30_000, {
      FFX_TERMINAL_TEST_FAIL_TMUX_CLOSE_CLEANUP: "1",
      FFX_TERMINAL_TEST_FAIL_SIGNAL_STAGE: "outside_group",
    });
    const firstStdout = streamText(firstHost.stdout);
    const firstStderr = streamText(firstHost.stderr);
    await waitFor(() => existsSync(paths.socket));
    ...
    execFileSync(
      "tmux",
      ["-S", tmuxSocket, "has-session", "-t", `ffx-${siblingIdentity}`],
    );
    success(await requestAction(
      first.client,
      first.revision!,
    ...
    expect(names).toEqual([`ffx-${siblingIdentity}`]);
    success(await requestAction(
      recovered.client,
      recovered.revision!,
      341,
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    const paths = hostPaths(home);
    const firstHost = startHost(home, undefined, 30_000, {
      FFX_TERMINAL_TEST_INTERRUPT_CLOSE_AFTER_COMMIT: "1",
    });
    const firstStdout = streamText(firstHost.stdout);
    const firstStderr = streamText(firstHost.stderr);
    await waitFor(() => existsSync(paths.socket));
    ...
    execFileSync(
      "tmux",
      ["-S", tmuxSocket, "has-session", "-t", `ffx-${closingIdentity}`],
    );
    execFileSync(
      "tmux",
      ["-S", tmuxSocket, "has-session", "-t", `ffx-${siblingIdentity}`],
...
test(
  ...
  async () => {
    ...
    const replacement = startHost(home, undefined, 300, {
      ...
      FFX_TRACE_SCOPES: "terminal_host",
      FFX_TERMINAL_TEST_ORDER_BARRIER: barrier,
      FFX_TERMINAL_TEST_ORDER_HOLD_CORRELATION: "335",
      FFX_TERMINAL_TEST_FAIL_CANCELLATION_OPEN: "1",
    });
    await waitFor(() =>
      existsSync(paths.socket) && existsSync(paths.identity) &&
      readFileSync(paths.identity, "utf8") !== oldIdentity
...
test("host remains authoritative until natural backend cleanup finishes", async () => {
  ...
  const host = startHost(home, undefined, 40, {
    FFX_TERMINAL_TEST_BACKEND_CLEANUP_DELAY_MS: "500",
  });
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
  const started = await startNativeCommandFixture(
...
test("process-token capture failure kills and reaps before returning failure", async () => {
  ...
  const host = startHost(
    ...
    { FFX_TERMINAL_FIXTURE_FAIL_PROCESS_TOKEN: "1" },
    buildCurrentClientFixture(),
  );
  await waitFor(() => existsSync(paths.socket));
  const connected = await handshake(paths.socket, { minimum: 4, current: 5 });
...
test("official short-lived clients preserve host-wide mutation order and inspect acknowledgement", async () => {
  ...
  const result = await runClientFixture(home, 400, {
    FFX_TERMINAL_OUTCOME_FIXTURE: "ordering",
    FFX_TERMINAL_TEST_ORDER_BARRIER: barrier,
    FFX_TERMINAL_TEST_ORDER_HOLD_CORRELATION: "3",
    FFX_TERMINAL_TEST_ORDER_HOLD_CORRELATION_2: "7",
  });
...
test("official client retains every reserved outcome through the exact capacity boundary", async () => {
  ...
  const result = await runClientFixture(home, 300, {
    FFX_TERMINAL_OUTCOME_FIXTURE: "retention",
  });
...
test.each([
  ...
  const paths = hostPaths(home);
  const result = await runClientFixture(home, 300, {
    FFX_TERMINAL_OUTCOME_FIXTURE: "failure",
    FFX_TERMINAL_TEST_HOST_FAILURE_POINT: point,
    FFX_TERMINAL_TEST_HOST_FAILURE_CORRELATION: "1",
  });
...
test("long-home transport roots reject symlink and non-private components without mutation", async () => {
  ...
  const outside = mkdtempSync(join(tmpdir(), "ffx-terminal-foreign-runtime-"));
  homes.push(outside);
...
test("fresh private client reloads owner-scoped authority without retaining proof", async () => {
  ...
  const started = await runClientFixture(home, 700, {
    FFX_TERMINAL_AUTHORITY_FIXTURE: "start",
  });
  ...
  const reloaded = await runClientFixture(home, 700, {
    FFX_TERMINAL_AUTHORITY_FIXTURE: "reload",
    FFX_TERMINAL_AUTHORITY_SESSION_ID: startValue.session_id,
  });
...
test("current client rejects same revision host without complete signal capability before start", async () => {
  ...
  const rejected = await runClientFixture(home, 700, {
    FFX_TERMINAL_CAPABILITY_FIXTURE: "start",
  });
...
test("current client permits graceful close and rejects force close on signal limited host", async () => {
  ...
  const forceRejected = await runClientFixture(home, 1_500, {
    FFX_TERMINAL_CAPABILITY_FIXTURE: "force_close",
    FFX_TERMINAL_AUTHORITY_FIXTURE_COMPAT: "1",
    FFX_TERMINAL_AUTHORITY_SESSION_ID: sessionId,
  });
...
test("protocol fixtures advertise exact evidence and interoperate in both directions", async () => {
  ...
  console.log("FFX_TERMINAL_COMPATIBILITY_EVIDENCE " + JSON.stringify({
    fixtures: advertised,
    directions: directionEvidence,
    active_current: {
      source: "FFX_BIN",
      digest: createHash("sha256").update(readFileSync(FFX_BIN)).digest("hex"),
    },
  }));
}, 360_000);
