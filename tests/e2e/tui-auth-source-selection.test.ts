...
import { FFX_BIN, REPO_ROOT, runFx } from "../evals/eval-helpers";
import {
  FAKE_GATEWAY_MODEL,
...
if (process.env.FFX_REQUIRE_TMUX === "1" && !HAS_TMUX) {
...
const LOGIN_TOKEN = "ffx-login-token";
...
async function startFx(
  ...
  return TmuxSession.create({
    ...
    env: {
      ...
      FFX_E2E_GATEWAY_MODELS_URL: `${fakeGateway.baseUrl}/coding-agent/v1/models`,
      FFX_MODEL: FAKE_GATEWAY_MODEL,
      FFX_AUTO_UPGRADE: "0",
      FFX_NO_OPEN_BROWSER: "1",
      FFX_OAUTH_CLIENT_ID: "test-client",
      FFX_E2E_OAUTH_ISSUER_URL: oauthIssuerUrl,
      FFX_TRACE_LOG: tracePath,
      FFX_TRACE_SCOPES: tracePath ? "auth,prompt" : undefined,
      ...envOverrides,
    },
...
function startFakeChatGptOAuth(
  ...
  return {
    ...
    requests,
    env: {
      FFX_E2E_CHATGPT_ISSUER_URL: baseUrl,
      FFX_E2E_CHATGPT_TOKEN_URL: `${baseUrl}/chatgpt/token`,
      FFX_E2E_OPENAI_CODEX_MODELS_URL: `${baseUrl}/chatgpt/models`,
      FFX_E2E_OPENAI_CODEX_RESPONSES_URL: `${baseUrl}/chatgpt/responses`,
    },
    baseUrl,
...
function startFakeGrokOAuth(options: {
  ...
  const server = Bun.serve({
    ...
    async fetch(request) {
      ...
      if (url.pathname === "/oauth2/authorize") {
        ...
        if (url.searchParams.get("referrer") !== "ffx") {
          return new Response("missing ffx referrer", { status: 400 });
        }
  ...
  return {
    ...
    baseUrl,
    env: {
      FFX_E2E_GROK_ISSUER_URL: baseUrl,
      FFX_E2E_GROK_TOKEN_URL: `${baseUrl}/oauth2/token`,
      FFX_E2E_GROK_USERINFO_URL: `${baseUrl}/oauth2/userinfo`,
      FFX_E2E_GROK_REVOKE_URL: `${baseUrl}/oauth2/revoke`,
      FFX_E2E_XAI_GROK_MODELS_URL: `${baseUrl}/v1/models`,
      FFX_E2E_XAI_GROK_MODALITIES_URL: `${baseUrl}/v1/language-models`,
      FFX_E2E_XAI_GROK_RESPONSES_URL: `${baseUrl}/v1/responses`,
    },
    setModels(next: typeof models) {
      models = next;
...
async function runGrokLoginWithBrowser(env: Record<string, string | undefined>) {
  ...
  const proc = nodeSpawn(FFX_BIN, ["login", "grok"], {
    cwd: REPO_ROOT,
    env: childEnv,
...
async function runCodexLoginWithBrowser(
  ...
  const proc = nodeSpawn(FFX_BIN, ["login", "codex"], {
    cwd: REPO_ROOT,
    env: childEnv,
    stdio: ["ignore", "pipe", "pipe"],
  });
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-inline-login-"));
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-chatgpt-cancel-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    ...
    expect(session.isAlive()).toBe(true);
    expect(existsSync(join(home, ".ffx", "chatgpt-auth.json"))).toBe(false);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-chatgpt-success-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([], {
      models() {
    ...
    const authPath = join(home, ".ffx", "chatgpt-auth.json");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-chatgpt-login-activation-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    chatgptOauth = startFakeChatGptOAuth();
    ...
    session = await startFx(
      ...
      undefined,
      undefined,
      { ...chatgptOauth.env, FFX_MODEL: undefined },
    );
    await session.waitForComposer(TIMEOUT);
    await session.sendText("/login");
    await session.waitForText("Sign in with Codex", TIMEOUT);
    ...
    const selected = JSON.parse(readFileSync(join(home, ".ffx", "settings.json"), "utf8"));
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-chatgpt-response-cancel-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-setup-hub-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN, undefined, 3600, 1, {
    ...
    expect(root).not.toContain("ffx login");
    ...
    expect(sources).toContain("ffx login");
    await session.sendKeys("Escape");
    await session.sendKeys("Escape");
    await session.waitForComposer(TIMEOUT);
...
function savedCredentialSource(testHome: string): string | undefined {
  const settingsPath = join(testHome, ".ffx", "settings.json");
...
profileStoredKeyTmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-stored-key-preference-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    ...
    session = await startFx(home, stderrPath, gateway, undefined, undefined, {
      FFX_DISABLE_KEYCHAIN: undefined,
    });
    await session.waitForComposer(TIMEOUT);
    await session.sendText("/status");
    ...
    const keyPath = join(home, ".ffx", "api-key");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-direct-login-preference-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([
    ...
    await session.sendText("/status");
    await session.waitForText("auth=ffx login", TIMEOUT);
...
tmuxTest(
  "Change team activates and persists ffx login ahead of the environment",
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-team-preference-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    ...
    const savedAuth = JSON.parse(readFileSync(join(home, ".ffx", "auth.json"), "utf8")) as {
      team_id?: string;
      team_slug?: string;
    };
...
tmuxTest(
  "API key and ffx login coexist through selection, restart, login, and logout",
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-lifecycle-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    ...
    const inventory = await session.waitForPane(
      (pane) => pane.includes("AI_GATEWAY_API_KEY") && pane.includes("ffx login"),
      TIMEOUT,
    );
    ...
    await session.waitForComposer(TIMEOUT);
    await session.sendText("/status");
    // The switch above is remembered, so the restart keeps ffx login rather than
    ...
    const loggedOut = await session.waitForText("Signed out of ffx.", TIMEOUT);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-team-models-"));
    stderrPath = join(home, "stderr.log");
    ...
    session = await startFx(
      ...
      tracePath,
      {
        ...
        FFX_TRACE_SCOPES: "auth,prompt,catalog",
      },
    );
    await session.waitForComposer(TIMEOUT);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-catalog-lifecycle-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
    ...
    await session.sendText("/logout");
    await session.waitForText("Signed out of ffx.", TIMEOUT);
    await waitForModelRequestCount(gateway, 3);
    expect(gateway.modelRequests).toHaveLength(3);
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-codex-cli-login-"));
    gateway = startFakeGateway([]);
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-grok-cli-login-"));
    gateway = startFakeGateway([]);
    ...
    try {
      ...
      const authPath = join(home, ".ffx", "grok-auth.json");
      expect(existsSync(authPath)).toBe(true);
      expect(statSync(authPath).mode & 0o077).toBe(0);
      const settings = JSON.parse(readFileSync(join(home, ".ffx", "settings.json"), "utf8"));
...
test("Grok logout removes local credentials when remote revocation fails", async () => {
  home = mkdtempSync(join(tmpdir(), "ffx-grok-logout-revoke-failure-"));
  ...
  try {
    ...
    const result = await runFx(["logout", "grok"], {
      env: {
        ...
        FFX_DISABLE_KEYCHAIN: "1",
        FFX_AUTO_UPGRADE: "0",
        FFX_E2E_GROK_REVOKE_URL: grok.env.FFX_E2E_GROK_REVOKE_URL,
      },
      timeoutMs: TIMEOUT,
    });
    ...
    expect(existsSync(authPath)).toBe(false);
    expect(JSON.parse(readFileSync(join(home, ".ffx", "settings.json"), "utf8")).provider)
      .toBe("grok");
    const ask = await runFx(["ask", "--json", "--no-save", "Still Grok?"], {
      env: { HOME: home, FFX_DISABLE_KEYCHAIN: "1", FFX_AUTO_UPGRADE: "0" },
      timeoutMs: TIMEOUT,
    });
    expect(ask.code).toBe(1);
    expect(ask.stderr).toContain("ffx login grok");
  } finally {
    grok.stop();
  }
});
...
test("Grok 401 replay refuses a different account before the second provider send", async () => {
  home = mkdtempSync(join(tmpdir(), "ffx-grok-account-mismatch-"));
  gateway = startFakeGateway([]);
  ...
  try {
    ...
    const saved = JSON.parse(readFileSync(join(home, ".ffx", "grok-auth.json"), "utf8")) as {
      access_token: string;
      account_id: string;
    };
...
test("Grok CLI sends verified images directly without advertising the vision fallback", async () => {
  home = mkdtempSync(join(tmpdir(), "ffx-grok-native-image-"));
  gateway = startFakeGateway([]);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-grok-tui-switch-"));
    stderrPath = join(home, "stderr.log");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-grok-effort-selection-"));
    stderrPath = join(home, "stderr.log");
    gateway = startFakeGateway([]);
    const grok = startFakeGrokOAuth();
    try {
      ...
      expect(readFileSync(join(home, ".ffx", "settings.json"), "utf8")).toContain('"effort":"xhigh"');
      expect(readFileSync(stderrPath, "utf8")).toBe("");
    } finally {
      grok.stop();
    }
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-grok-resource-recovery-"));
    stderrPath = join(home, "stderr.log");
    gateway = startFakeGateway([]);
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-chatgpt-tool-loop-"));
    gateway = startFakeGateway([]);
    ...
    try {
      ...
      const result = await runFx(
        ["ask", "--json", "--auto", "--no-save", "Read the README, then finish."],
        {
          env: {
            HOME: home,
            FFX_PROVIDER_API_KEY: "gateway-tool-loop-sentinel",
            VERCEL_OIDC_TOKEN: undefined,
            FFX_DISABLE_KEYCHAIN: "1",
            FFX_AUTO_UPGRADE: "0",
            FFX_GATEWAY_BASE_URL: gateway.baseUrl,
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-grok-tool-loop-"));
    gateway = startFakeGateway([]);
    ...
    try {
      ...
      const result = await runFx(
        ["ask", "--json", "--auto", "--no-save", "Read the README, then finish."],
        {
          env: {
            HOME: home,
            FFX_PROVIDER_API_KEY: "gateway-grok-tool-loop-sentinel",
            VERCEL_OIDC_TOKEN: undefined,
            FFX_DISABLE_KEYCHAIN: "1",
            FFX_AUTO_UPGRADE: "0",
            FFX_GATEWAY_BASE_URL: gateway.baseUrl,
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-codex-cli-activation-failure-"));
    gateway = startFakeGateway([]);
    chatgptOauth = startFakeChatGptOAuth();
    chatgptOauth.setModels([]);
    const env = {
    ...
    expect(login.stderr).toContain("ffx login: could not load the target model catalog (malformed_response)");
    expect(existsSync(join(home, ".ffx", "chatgpt-auth.json"))).toBe(true);
    const settingsPath = join(home, ".ffx", "settings.json");
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-grok-cli-activation-failure-"));
    gateway = startFakeGateway([]);
    const grok = startFakeGrokOAuth();
    grok.setModels([]);
    try {
      ...
      expect(login.stderr).toContain("ffx login: target model catalog is empty");
      expect(existsSync(join(home, ".ffx", "grok-auth.json"))).toBe(true);
      expect(existsSync(join(home, ".ffx", "settings.json"))).toBe(false);
    } finally {
      grok.stop();
    }
  },
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-codex-vision-disabled-"));
    gateway = startFakeGateway([]);
    const codex = startFakeCodexToolLoop({
      toolName: "vision",
    ...
    try {
      ...
      const result = await runFx(
        ["ask", "--json", "--auto", "--no-save", "Answer without using a vision fallback."],
        {
          env: {
            HOME: home,
            FFX_PROVIDER_API_KEY: "gateway-vision-sentinel",
            VERCEL_OIDC_TOKEN: undefined,
            FFX_DISABLE_KEYCHAIN: "1",
            FFX_AUTO_UPGRADE: "0",
            FFX_GATEWAY_BASE_URL: gateway.baseUrl,
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-grok-vision-disabled-"));
    gateway = startFakeGateway([]);
    const grok = startFakeGrokToolLoop({
      toolName: "vision",
      toolArguments: { image_ids: [1], focus: "Inspect the image." },
    ...
    try {
      ...
      const result = await runFx(
        ["ask", "--json", "--auto", "--no-save", "Answer without a vision fallback."],
        {
          env: {
            HOME: home,
            FFX_PROVIDER_API_KEY: "gateway-grok-vision-sentinel",
            VERCEL_OIDC_TOKEN: undefined,
            FFX_DISABLE_KEYCHAIN: "1",
            FFX_AUTO_UPGRADE: "0",
            FFX_GATEWAY_BASE_URL: gateway.baseUrl,
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-codex-auto-review-"));
    gateway = startFakeGateway([]);
    ...
    try {
      ...
      const result = await runFx(
        ["ask", "--json", "--auto", "--no-save", "Run pwd, then finish."],
        {
          env: {
            HOME: home,
            FFX_PROVIDER_API_KEY: "gateway-auto-review-sentinel",
            VERCEL_OIDC_TOKEN: undefined,
            FFX_DISABLE_KEYCHAIN: "1",
            FFX_AUTO_UPGRADE: "0",
            FFX_GATEWAY_BASE_URL: gateway.baseUrl,
...
test(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-grok-auto-review-"));
    gateway = startFakeGateway([]);
    ...
    try {
      ...
      const result = await runFx(
        ["ask", "--json", "--auto", "--no-save", "Run pwd, then finish."],
        {
          env: {
            HOME: home,
            FFX_PROVIDER_API_KEY: "gateway-grok-auto-review-sentinel",
            VERCEL_OIDC_TOKEN: undefined,
            FFX_DISABLE_KEYCHAIN: "1",
            FFX_AUTO_UPGRADE: "0",
            FFX_GATEWAY_BASE_URL: gateway.baseUrl,
      ...
      for (const headers of grok.headers) {
        ...
        expect(headers.clientIdentifier).toBe("ffx");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-preflight-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    const imagePath = join(home, "attachment.png");
    writeFileSync(imagePath, Buffer.from("89504e470d0a1a0a72657374", "hex"));
    ...
    expect(blocked).not.toContain("Welcome to ffx");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-logout-active-login-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    ...
    await session.sendKeys("Down");
    await session.sendKeys("Enter");
    await session.sendText("prove ffx login is active before logout");
    await session.waitForText(LOGIN_RESPONSE, TIMEOUT);
    expect(gateway.requests[0].headers.get("authorization")).toBe(`Bearer ${LOGIN_TOKEN}`);
    ...
    const loggedOut = await session.waitForPane(
      (pane) =>
        pane.includes("Signed out of ffx.") &&
    ...
    expect(inventory).not.toMatch(/^\s+ffx login\s+(?:current|available)\s*$/m);
    await session.sendKeys("Escape");
...
tmuxTest(
  "logout preserves an active API key when ffx login is inactive",
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-logout-inactive-login-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
...
tmuxTest(
  "logout removes an ffx login rejected for unsafe permissions",
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-logout-rejected-login-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-logout-delete-failure-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([fakeGatewayFinalText(LOGIN_RESPONSE)]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
    ...
    const failed = await session.waitForText(
      "Could not confirm durable ffx logout. The active source was recalculated.",
      TIMEOUT,
    );
    expect(failed).not.toContain("Signed out of ffx.");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-logout-only-login-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
    ...
    expect(inventory).not.toMatch(/^\s+ffx login\s+/m);
    await session.sendKeys("Escape");
    await session.waitForPane(
    ...
    const picker = await session.waitForPane(
      (pane) =>
        ...
        pane.includes("Welcome to ffx") &&
        pane.includes("Sign in with Vercel") &&
        pane.includes("Add an API key") &&
        pane.includes("Esc to set up later"),
      TIMEOUT,
    );
    expect(picker).not.toMatch(/^\s+ffx login\s+/m);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-http-failure-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-source-failure-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    writeFileSync(stderrPath, "");
    ...
    const failed = await session.waitForPane(
      (pane) =>
        pane.includes("ffx login credential refresh failed.") &&
    ...
    const preserved = await session.waitForPane(
      (pane) =>
        ...
        !pane.includes("current: ffx login"),
      TIMEOUT,
    );
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-refresh-team-models-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([fakeGatewayFinalText(REFRESH_RECOVERY_RESPONSE)], {
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-expired-models-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-ready-catalog-expiry-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([], {
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-expired-refresh-models-"));
    stderrPath = join(home, "stderr.log");
    const tracePath = join(home, "trace.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-expired-credits-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
    ...
    session = await startFx(
      ...
      {
        AI_GATEWAY_API_KEY: undefined,
        FFX_E2E_GATEWAY_CREDITS_URL: creditsGateway.url,
      },
    );
    await session.waitForComposer(TIMEOUT);
    await waitForModelRequestCount(gateway, 1);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-expired-startup-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(null);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-models-before-login-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([]);
    oauth = startFakeOAuth(ACQUIRED_LOGIN_TOKEN);
...
tmuxTest(
  ...
  async () => {
    home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-populated-catalog-fallback-"));
    stderrPath = join(home, "stderr.log");
    writeFileSync(stderrPath, "");
    gateway = startFakeGateway([], {
      models(request) {
...
for (const scenario of [
  ...
  tmuxTest(
    ...
    async () => {
      home = mkdtempSync(join(tmpdir(), "ffx-tui-auth-empty-catalog-"));
      stderrPath = join(home, "stderr.log");
      writeFileSync(stderrPath, "");
      gateway = startFakeGateway([], {
        models(request) {
