...
function latestTraceReportPath(root: IsolatedRoot): string {
  const reports = readdirSync(root.root)
    .filter((entry) => entry.startsWith("ffx-trace-") && entry.endsWith(".md"))
    .map((entry) => {
...
function createIsolatedRoot(baseDir = tmpdir()): IsolatedRoot {
  const root = realpathSync(mkdtempSync(join(baseDir, "ffx-command-permissions-e2e-")));
...
function gatewayEnv(
  ...
  return {
    HOME: root.home,
    FFX_PROVIDER_API_KEY: "fake-command-permission-key",
    VERCEL_OIDC_TOKEN: undefined,
    FFX_GATEWAY_BASE_URL: gateway.baseUrl,
    FFX_GATEWAY_CHAT_URL: gateway.chatUrl,
    FFX_MODEL: MODEL,
    FFX_AUTO_UPGRADE: "0",
    FFX_DIRECT_SECRET: "must-not-be-inherited",
    NO_COLOR: "1",
    ...extra,
  };
}
...
async function launchPermissionResumeHarness(initialResponses: Response[]) {
  ...
  const initialSession = await TmuxSession.create({
    cmd: FFX_BIN,
    cwd: root.workspace,
    env: gatewayEnv(root, initialGateway, { FFX_PERMISSION_MODE: undefined }),
  ...
  return {
    ...
    async resume(responses: Response[]) {
      ...
      const session = await TmuxSession.create({
        ...
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, { FFX_PERMISSION_MODE: undefined }),
...
function commandReplayFiles(root: IsolatedRoot): string[] {
  ...
  const result = Bun.spawnSync(
    ["find", sessions, "-type", "f", "-name", "ffx-command-replay-*"],
...
function largeEffectfulCommand(marker: string) {
  const command = [
    ...
    `printf '%s\\n' FFX_LARGE_RUN_COMMAND_DONE > ${marker}`,
  ].join("\n");
...
describe("effect-aware command permissions", () => {
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          FFX_TRACE_SCOPES: "gateway,permission,session,tool",
        }),
        stderrPath,
        width: 100,
        height: 28,
      ...
      await activeSession.waitForText("Yes, and tell ffx what to do next", TIMEOUT);
      ...
      const events = readFileSync(
        join(root.home, ".ffx", "sessions", sessionId, "events.jsonl"),
        "utf8",
      );
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          FFX_PERMISSION_MODE: "yolo",
          FFX_TRACE_LOG: tracePath,
          FFX_TRACE_SCOPES: "core",
        }),
        stderrPath,
        width: 120,
        height: 40,
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          FFX_PERMISSION_MODE: "yolo",
          FFX_TRACE_LOG: tracePath,
          FFX_TRACE_SCOPES: "core,permission,tool",
        }),
        stderrPath,
        width: 120,
        height: 40,
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          FFX_TRACE_LOG: join(root.root, "minimal-command-output-trace.log"),
          FFX_TRACE_SCOPES: "core,agent,tool,session,command_output",
        }),
        stderrPath,
        width: 120,
        height: 36,
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          FFX_PERMISSION_MODE: "yolo",
          FFX_TRACE_LOG: tracePath,
          FFX_TRACE_SCOPES: "core,tool,session,command_output",
        }),
        stderrPath,
        width: 72,
        height: 30,
      ...
      expect(publicSession.stdout).not.toContain("ffx-command-replay-");
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        cmd: FFX_BIN,
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, { FFX_PERMISSION_MODE: "yolo" }),
        stderrPath,
        width: 90,
        height: 30,
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      installClipboardFixture(
        root,
        '#!/bin/sh\nfor arg in "$@"; do last="$arg"; done\nprintf "%s" "$last" > "$FFX_TRACE_CLIPBOARD_OUTPUT"\n',
      );
      writeFileSync(stderrPath, "");
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          PATH: hostilePath(root),
          TMPDIR: root.root,
          FFX_TRACE_CLIPBOARD_OUTPUT: clipboardPath,
        }),
        stderrPath,
        width: 120,
        height: 40,
      ...
      expect(escapes).not.toContain("ffx.sh/feedback");
      expect(escapes).not.toContain("github.com");
      const reportPath = latestTraceReportPath(root);
      const report = readFileSync(reportPath, "utf8");
      expect(report).toContain("# ffx trace");
  ...
  test.skipIf(!tmuxAvailable())(
    "TUI feedback opens ffx.sh without creating a trace or touching the clipboard",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([]);
      ...
      installUrlOpenerFixture(
        root,
        '#!/bin/sh\nprintf "%s" "$1" > "$FFX_FEEDBACK_OPEN_OUTPUT"\n',
      );
      installClipboardFixture(
        root,
        '#!/bin/sh\nprintf used > "$FFX_FEEDBACK_CLIPBOARD_MARKER"\n',
      );
      writeFileSync(stderrPath, "");
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          PATH: hostilePath(root),
          TMPDIR: root.root,
          FFX_FEEDBACK_OPEN_OUTPUT: openerPath,
          FFX_FEEDBACK_CLIPBOARD_MARKER: clipboardMarker,
        }),
        stderrPath,
        width: 120,
        height: 40,
      ...
      await activeSession.waitForText("Opened https://ffx.sh/feedback.", TIMEOUT);

      expect(readFileSync(openerPath, "utf8")).toBe("https://ffx.sh/feedback");
      expect(existsSync(clipboardMarker)).toBe(false);
      expect(
        readdirSync(root.root).filter((entry) => entry.startsWith("ffx-trace-")),
      ).toHaveLength(0);
      const escapes = await activeSession.capturePaneEscapes();
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      const binary = process.env.FFX_COMMAND_SESSION_TEST_BIN ?? FFX_BIN;
      ...
      for (const sandbox of sandboxModes) {
        ...
        activeSession = await TmuxSession.create({
          ...
          env: gatewayEnv(root, gateway, {
            ...
            FFX_RECORD_INPUT: "1",
            FFX_TRACE_LOG: tracePath,
            FFX_TRACE_SCOPES: "agent,core,gateway,permission,session,tool,worker",
          }),
          width: 120,
          height: 40,
          minimumHistoryLines: 1_000,
        ...
        await activeSession.sendText(
          "export PS1='FFX_OUTER_PROMPT> '; printf 'FFX_OUTER_SHELL_READY\\n'",
        );
        await activeSession.waitForText("FFX_OUTER_SHELL_READY", TIMEOUT);
        await activeSession.sendText(
        ...
        expect(gateway.requests[1]!.body).not.toContain("\\u0006");
        expect(gateway.requests[1]!.body).not.toContain("\\u0000");
        expect(gateway.requests[1]!.body).not.toContain("FFX_FOREGROUND_EXEC_FAILED");
        ...
        expect(scrollback).not.toContain("FFX_FOREGROUND_EXEC_FAILED");
        ...
        expect(replay.stdout).not.toContain("FFX_FOREGROUND_EXEC_FAILED");
      }
    },
    90_000,
  );
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          FFX_PERMISSION_MODE: "auto",
          FFX_TRACE_LOG: tracePath,
          FFX_TRACE_SCOPES: "permission,interrupt",
        }),
        stderrPath,
        width: 120,
        height: 40,
  ...
  test(
    "default ffx ask defaults missing permission mode to auto",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask yolo returns repeated user-profile command results to the model",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask yolo completes more than ten serial user-profile commands when unlimited",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        ...Array.from(
  ...
  test(
    "ffx ask condition-waits for a canonical child without shell polling",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask pauses and resumes provider recovery for a child",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask runs repeated turns for a persistent canonical subagent",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask delivers periodic child notifications at the next available parent step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask delivers two ordered child messages at one parent boundary",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask resumes a 64 KiB child message through five bounded projections",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask permits a child to create a nested canonical child",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask nested child receives periodic grandchild delivery at the next available step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive ffx delivers a child approval to the next same-turn parent step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive ffx lets a child finish automatic recovery without parent approval",
    async () => {
      const root = createIsolatedRoot();
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          FFX_TRACE_LOG: tracePath,
          FFX_TRACE_SCOPES: "scroll,frame_commit",
        }),
        stderrPath,
        width: 120,
        height: 36,
      ...
      expect(JSON.parse(readFileSync(join(root.home, ".ffx", "settings.json"), "utf8")).permission_mode)
        .toBe("ask");
      expect(gateway.requests).toHaveLength(1);
      expect(activeSession.isAlive()).toBe(true);
      expect(readFileSync(stderrPath, "utf8")).toBe("");
  ...
  test(
    "ffx ask yolo executes pwd through the default user profile without an artifact",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask defaults missing permission mode to auto through the classifier",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask does not retry a malformed classifier completion and safely replans",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask returns one malformed classifier completion to the agent without execution",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask provider failure never executes or enters malformed recovery",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask SIGINT during classifier wait terminates before decision or execution",
    async () => {
      const root = createIsolatedRoot();
      ...
      const child = nodeSpawn(
        ...
        ["ask", "--quiet", "--json", "--no-save", "Run the classifier cancellation fixture."],
        {
          ...
          env: definedEnv(gatewayEnv(root, gateway, {
            FFX_TRACE_LOG: tracePath,
            FFX_TRACE_SCOPES: "permission,stream",
          })),
      ...
      try {
        ...
        const result = await Promise.race([
          ...
          Bun.sleep(2_000).then(() => {
            throw new Error("ffx did not exit on SIGINT while the classifier remained blocked");
          }),
        ]);
  ...
  test(
    "ffx ask automatic review receives the exact delegated command",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "ffx ask terminal automatic ask returns a recoverable denial without prompting",
    async () => {
      const root = createIsolatedRoot();
      const marker = join(root.workspace, "ffx-ask-prompt-approved.txt");
      const command = `printf approved > ${JSON.stringify(marker)}`;
      const gateway = startFakeGateway(
        [
          toolCall(command),
          finalText("ffx ask prompt complete"),
        ],
        { classifierDecision: "ask" },
      );
      const tracePath = join(root.root, "trace.log");
      ...
      activeSession = await TmuxSession.create({
        cmd: `${shellQuote(FFX_BIN)} ask --auto --no-save ${shellQuote("Run the one-shot prompt fixture.")}`,
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, {
          FFX_TRACE_LOG: tracePath,
          FFX_TRACE_SCOPES: "permission",
      ...
      const finalPane = await activeSession.waitForText("ffx ask prompt complete", TIMEOUT);
  ...
  test(
    "ffx ask and ACP send large automatic review packets before execution",
    async () => {
  ...
  test(
    "ffx ask projects hostile ls filenames through the default user profile",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "ffx ask preserves quoted shell metacharacters through the user profile",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        toolCall("printf '%s' '<'"),
  ...
  test(
    "ffx ask keeps parser hardening cases approval-bearing",
    async () => {
      const commands = [
        "wc -c < input.txt",
        "printf x\r|wc -c",
  ...
  test(
    "ffx ask blocks approval-bearing commands before side effects",
    async () => {
      const root = createIsolatedRoot();
      const marker = join(root.workspace, "must-not-exist");
  ...
  test(
    "ffx ask blocks hostile git before any executable or repository access",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        toolCall("git status"),
