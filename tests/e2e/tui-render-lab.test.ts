...
test("render-lab lists gated Plan B native scenarios", () => {
  ...
  expect(output).toContain("FFX_RENDER_LAB_NATIVE=1");
});

test.skipIf(!existsSync(FFX_BIN))("buffer-system frame benchmark writes timing artifact", () => {
  outDir = mkdtempSync(join(tmpdir(), "ffx-frame-bench-test-"));
...
test.skipIf(!existsSync(FFX_BIN))("buffer-system p95 gate rejects undersampled runs", () => {
  outDir = mkdtempSync(join(tmpdir(), "ffx-frame-bench-samples-test-"));
...
test("native render-lab scenarios require explicit opt-in", () => {
  outDir = mkdtempSync(join(tmpdir(), "ffx-render-lab-gate-test-"));
  const env = { ...process.env };
  delete env.FFX_RENDER_LAB_NATIVE;
  delete env.FFX_RENDER_LAB_NATIVE_COMMAND_K;
  delete env.FFX_RENDER_LAB_NATIVE_ALLOW_CLIPBOARD;
  ...
  expect(result.stderr).toContain("FFX_RENDER_LAB_NATIVE=1");
});

test("render-lab analyzer flags excess gap before transient activity", () => {
  outDir = mkdtempSync(join(tmpdir(), "ffx-render-lab-analyzer-test-"));
...
test("render-lab analyzer rejects damaged multiline footer with real hint and cursor", () => {
  ...
  const manifest = writeAnalyzerFixture(outDir, [
    ...
    "  transcript row two",
    "────────────────",
    "run ffx login · opus 4.7 · default",
  ], { row: 3, col: 8 });
...
describe.skipIf(SKIP)("tui: render lab", () => {
  ...
  test(
    ...
    () => {
      outDir = mkdtempSync(join(tmpdir(), "ffx-render-lab-test-"));
      const env = { ...process.env };
      ...
      expect(FFX_BIN).toContain("zig-out/bin/ffx");
      const runDir = output.trim().split(/\s+/).at(-1)!;
...
function runScenarioArtifacts(scenario: string): {
  ...
  outDir = mkdtempSync(join(tmpdir(), "ffx-render-lab-overflow-test-"));
  const env = { ...process.env };
  delete env.AI_GATEWAY_API_KEY;
  delete env.VERCEL_OIDC_TOKEN;
  const output = execFileSync(
