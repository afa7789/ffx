...
const TRANSIENT_ACTIVITY_CAPTURE_TARBALL = "/private/tmp/ffx-render-bug-20260510-073148.tar.gz";
const TRANSIENT_ACTIVITY_CAPTURE_DIR = "/private/tmp/ffx-render-bug-20260510-073148";
const READ_ONLY_TOOLS_CAPTURE_TARBALL = join(
  import.meta.dirname,
  "fixtures",
  "ffx-render-bug-20260510-075848.tar.gz",
);
...
async function launch(options: {
  ...
  const workDir = mkdtempSync(join(tmpdir(), "ffx-render-replay-"));
  ...
  mkdirSync(join(workDir, ".ffx"), { recursive: true });
  writeFileSync(
    join(workDir, ".ffx", "settings.json"),
  ...
  const s = await TmuxSession.create({
    ...
    env: {
      HOME: workDir,
      FFX_RECORD: tapePath,
      ...(options.recordInput ? { FFX_RECORD_INPUT: "1" } : {}),
      ...(options.syncUpdates ? { FFX_SYNC_UPDATES: options.syncUpdates } : {}),
...
async function launchAutomaticRecording(): Promise<{
  ...
  const workDir = mkdtempSync("/tmp/ffx-render-auto-replay-");
  workDirs.push(workDir);
  ...
  const tracePath = join(workDir, "trace.log");
  const s = await TmuxSession.create({
    cmd: `env -u AI_GATEWAY_API_KEY -u VERCEL_OIDC_TOKEN FFX_DISABLE_KEYCHAIN=1 FFX_SKIP_ONBOARDING=1 ${FFX_BIN} --record`,
    cwd: workDir,
    width: 180,
    height: 36,
    env: {
...
describe("tui: render record/replay", () => {
  test.skipIf(!existsSync(TRANSIENT_ACTIVITY_CAPTURE_TARBALL) && !existsSync(join(TRANSIENT_ACTIVITY_CAPTURE_DIR, "bug.fxtape")))(
    ...
    () => {
      ...
      if (!existsSync(join(captureDir, "bug.fxtape"))) {
        const workDir = mkdtempSync(join(tmpdir(), "ffx-render-capture-"));
        workDirs.push(workDir);
        execFileSync("tar", ["-xzf", TRANSIENT_ACTIVITY_CAPTURE_TARBALL, "-C", workDir]);
        captureDir = join(workDir, "ffx-render-bug-20260510-073148");
      }

      const tapePath = join(captureDir, "bug.fxtape");
      const workDir = mkdtempSync(join(tmpdir(), "ffx-render-capture-replay-"));
      workDirs.push(workDir);
      const goldenPath = join(workDir, "grid.txt");
      const tracePath = join(workDir, "trace.log");

      const replayJsonOutput = execFileSync(FFX_BIN, ["replay", tapePath, "--json"], {
        encoding: "utf8",
      });
      ...
      execFileSync(FFX_BIN, ["replay", tapePath, "--golden", goldenPath], {
        env: {
          ...process.env,
          FFX_TRACE_LOG: tracePath,
          FFX_TRACE_SCOPES: TRACE_SCOPES,
  ...
  test.skipIf(SKIP)(
    ...
    async () => {
      ...
      execFileSync(FFX_BIN, ["replay", launched.tapePath, "--golden", launched.goldenPath]);
  ...
  test(
    ...
    () => {
      const failures: string[] = [];
      const extractDir = mkdtempSync(join(tmpdir(), "ffx-render-read-only-tools-"));
      workDirs.push(extractDir);
      execFileSync("tar", ["-xzf", READ_ONLY_TOOLS_CAPTURE_TARBALL, "-C", extractDir]);
      const captureDir = join(extractDir, "ffx-render-bug-20260510-075848");
      const tapePath = join(captureDir, "bug.fxtape");
      const workDir = mkdtempSync(join(tmpdir(), "ffx-render-read-only-tools-replay-"));
      workDirs.push(workDir);
      const goldenPath = join(workDir, "grid.txt");
      const tracePath = join(workDir, "trace.log");
  ...
  test.skipIf(SKIP)(
    ...
    async () => {
      ...
      expect(launched.tapePath.startsWith(join(launched.home, ".ffx", "recordings"))).toBe(true);
      await session.sendText(marker);
      await session.waitForText("Fx needs access to Vercel AI Gateway", 5_000);
