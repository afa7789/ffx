...
describe.skipIf(SKIP)("tui: resize", () => {
  test(
    ...
    async () => {
      ...
      const headerRow = grid.findIndex((line) => line.includes("𝒇𝒇x v"));
  ...
  test(
    ...
    async () => {
      ...
      expect(scrollback.match(/𝒇𝒇x v\d+\.\d+\.\d+\b/g)).toHaveLength(1);
  ...
  test(
    ...
    async () => {
      ...
      const transcriptCopyCounts = (scrollback: string) => ({
        startup: scrollback.match(/𝒇𝒇x v\d+\.\d+\.\d+\b/g)?.length ?? 0,
  ...
  test(
    ...
    async () => {
      ...
      expect(restored.match(/𝒇𝒇x v\d+\.\d+\.\d+\b/g)).toHaveLength(1);
