...
function makePaths(label: string): Paths {
  ...
  const root = realpathSync(mkdtempSync(join(tempRoot, `ffx-r-${label}-`)));
  return {
    root,
    home: join(root, "home"),
...
function gatewayEnv(home: string, gateway: ReturnType<typeof startFakeGateway>) {
  return {
    HOME: home,
    FFX_PROVIDER_API_KEY: "fake-resume-brutal-key",
...
async function seedRealSession(paths: Paths, config: Config): Promise<IndexedSummary> {
  ...
  const indexPath = join(paths.home, ".ffx", "sessions", "index.json");
  ...
  try {
    session = await TmuxSession.create({
      ...
      env: {
        ...
        FFX_TRACE_LOG: paths.trace,
        FFX_TRACE_SCOPES: "core,session",
      },
      stderrPath: paths.stderr,
      width: 112,
      height: 32,
    ...
    const report = {
      config,
      catalogBytes: statSync(join(paths.home, ".ffx", "sessions", "index.json")).size,
      transitions: {
...
test.skipIf(!tmuxAvailable() || process.env.FFX_RESUME_BRUTAL !== "1")(
...
test.skipIf(
  !tmuxAvailable() ||
    process.env.FFX_RESUME_PROFILE !== "1" ||
    platform() !== "darwin" ||
...
test.skipIf(!tmuxAvailable() || process.env.FFX_RESUME_50K !== "1")(
  "/resume survives a fifty-thousand-entry catalog and real fifty-thousand-line tool-heavy session",
  async () => {
    const profileSeconds = process.env.FFX_RESUME_PROFILE === "1" &&
        platform() === "darwin" &&
        existsSync("/usr/bin/sample")
      ? 30
      : undefined;
