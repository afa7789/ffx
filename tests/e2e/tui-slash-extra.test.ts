...
describe.skipIf(!tmuxAvailable())("tui: skills command recovery", () => {
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "ffx-invalid-skill-name-"));
      ...
      try {
        ...
        const recovered = await session.waitForText("ffx managed install root:", 5_000);
      ...
      try {
        session = await TmuxSession.create({
          env: {
            ...
            FFX_E2E_GATEWAY_CREDITS_URL: gateway.url,
          },
          width: 120,
          height: 40,
        });
...
describe.skipIf(!tmuxAvailable() || CLIPBOARD_PROGRAM === null)("tui: clipboard host", () => {
  test(
    ...
    async () => {
      ...
      const workDir = mkdtempSync(join(tmpdir(), "ffx-clipboard-host-"));
      ...
      try {
        session = await TmuxSession.create({
          ...
          env: {
            HOME: homeDir,
            FFX_PROVIDER_API_KEY: "clipboard-fake-key",
            VERCEL_OIDC_TOKEN: undefined,
            ...
            FFX_TEST_CLIPBOARD_CAPTURE: capturePath,
...
describe.skipIf(!tmuxAvailable())("tui: active session transitions", () => {
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "ffx-active-clear-"));
      const homeDir = mkdtempSync(join(tmpdir(), "ffx-active-clear-home-"));
      const stderrPath = join(workDir, "stderr.log");
      let requestCount = 0;
      ...
      try {
        session = await TmuxSession.create({
          ...
          env: {
            HOME: homeDir,
            FFX_PROVIDER_API_KEY: "active-clear-fake-key",
            VERCEL_OIDC_TOKEN: undefined,
            FFX_GATEWAY_BASE_URL: gateway.baseUrl,
            FFX_GATEWAY_CHAT_URL: gateway.chatUrl,
            FFX_E2E_GATEWAY_CHAT_URL: gateway.chatUrl,
...
describe.skipIf(SKIP)("tui: extra slash commands", () => {
  ...
  test(
    ...
    async () => {
      const workDir = mkdtempSync(join(tmpdir(), "ffx-row03-clear-"));
      const homeDir = mkdtempSync(join(tmpdir(), "ffx-row03-clear-home-"));
      ...
      try {
        session = await TmuxSession.create({
          ...
          env: {
            HOME: homeDir,
            FFX_PROVIDER_API_KEY: "clear-fake-key",
            VERCEL_OIDC_TOKEN: undefined,
            FFX_GATEWAY_BASE_URL: gateway.baseUrl,
            FFX_GATEWAY_CHAT_URL: gateway.chatUrl,
            FFX_E2E_GATEWAY_CHAT_URL: gateway.chatUrl,
  ...
  test(
    ...
    async () => {
      const home = mkdtempSync(join(tmpdir(), "ffx-usage-empty-home-"));
